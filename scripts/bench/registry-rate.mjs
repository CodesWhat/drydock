...
function main() {
  ...
  console.log(
    `Fleet: ${FLEET.length} containers (${hubContainers.length} Docker Hub, ${ghcrContainers.length} GHCR)`,
  );
  console.log(
    `  immich-server-class: ${FLEET.find((c) => c.name === 'immich-server').tagPages} tag pages each`,
  );
  console.log('');
  ...
  console.log(
    `    Hourly cadence:   ${H.hubPulls} pulls → ${pct(H.hubPulls, CEILINGS.hubPullsAuth)}% used, ${headroom(H.hubPulls, CEILINGS.hubPullsAuth)}`,
  );
  console.log(
    `    6-hourly cadence: ${SH.hubPulls} pulls → ${pct(SH.hubPulls, CEILINGS.hubPullsAuth)}% used, ${headroom(SH.hubPulls, CEILINGS.hubPullsAuth)}`,
  );
  console.log(`  Anonymous ceiling (100/6h):`);
  console.log(
    `    Hourly cadence:   ${H.hubPulls} pulls → ${pct(H.hubPulls, CEILINGS.hubPullsAnon)}% used, ${headroom(H.hubPulls, CEILINGS.hubPullsAnon)}`,
  );
  console.log(
    `    6-hourly cadence: ${SH.hubPulls} pulls → ${pct(SH.hubPulls, CEILINGS.hubPullsAnon)}% used, ${headroom(SH.hubPulls, CEILINGS.hubPullsAnon)}`,
  );
  console.log('');
  ...
  console.log(
    `  Speedup: ${(H.ghcrRegistry / SH.ghcrRegistry).toFixed(0)}× fewer requests against GHCR per 6h window`,
  );
  console.log('');

  console.log(
    `GitHub REST API release-notes (5,000/hr authenticated → ${CEILINGS.githubApi.toLocaleString()}/6h ceiling):`,
  );
  console.log(
    `  Hourly cadence:   ${H.githubApi} req → ${pct(H.githubApi, CEILINGS.githubApi)}% used, ${headroom(H.githubApi, CEILINGS.githubApi)}`,
  );
  console.log(
    `  6-hourly cadence: ${SH.githubApi} req → ${pct(SH.githubApi, CEILINGS.githubApi)}% used, ${headroom(SH.githubApi, CEILINGS.githubApi)}`,
  );
  console.log('');

  console.log('### Summary\n');
  console.log(
    `- Hub pull quota (auth 200/6h): hourly=${H.hubPulls} pulls (${pct(H.hubPulls, CEILINGS.hubPullsAuth)}%), 6-hourly=${SH.hubPulls} pulls (${pct(SH.hubPulls, CEILINGS.hubPullsAuth)}%) — comfortable headroom at both cadences`,
  );
  console.log(
    `- Hub pull quota (anon 100/6h): hourly=${H.hubPulls} pulls (${pct(H.hubPulls, CEILINGS.hubPullsAnon)}%), 6-hourly=${SH.hubPulls} pulls (${pct(SH.hubPulls, CEILINGS.hubPullsAnon)}%) — comfortable headroom at both cadences`,
  );
  console.log(
    `- GHCR registry calls: ${H.ghcrRegistry} → ${SH.ghcrRegistry} per 6h window (${(H.ghcrRegistry / SH.ghcrRegistry).toFixed(0)}× reduction) — where Cesc's 429s originated (#342)`,
  );
  console.log(
    `- GitHub API release-notes: ${H.githubApi} → ${SH.githubApi} per 6h window (${(H.githubApi / SH.githubApi).toFixed(0)}× reduction, ${pct(H.githubApi, CEILINGS.githubApi)}% → ${pct(SH.githubApi, CEILINGS.githubApi)}% of ceiling)`,
  );
  console.log(
