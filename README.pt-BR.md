...
| **v1.8.0**   | Gerenciamento de frota e configuração ao vivo | Configuração YAML, configuração de UI ao vivo, navegador de volume, atualizações paralelas, migração de armazenamento SQLite, cadeia de autenticação própria substituindo o Passport.js (já lançada), autenticação de dois fatores TOTP, progresso de atualização do Home Assistant e dispositivos por contêiner ([#210](https://github.com/CodesWhat/drydock/discussions/210)), imagens criadas localmente monitoradas em relação a uma base upstream declarada ([#897](https://github.com/CodesWhat/drydock/discussions/897)), chaves de API com escopo, com limites de taxa por chave e revogação em cascata, para integrações HA/painel ([#469](https://github.com/CodesWhat/drydock/discussions/469)), fila de aprovação por atualização |
