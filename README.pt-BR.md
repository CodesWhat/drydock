...
**Observador de atualização de imagem de contêiner, 23 registros, 21 provedores de notificação e ação.**
...
| Roadmap                | Consulte a seção [Roteiro](#roadmap) abaixo                                                                                  |
...
> **Nota:** O Compose trata `$` como sintaxe de interpolação de variáveis, então um hash argon2id colado com um único `$` chega ao Drydock corrompido. Duplique cada `$` para `$$` ao colar o hash real, por exemplo `$$argon2id$$v=19$$m=65536,t=3,p=4$$salt$$hash`.
...
>
> Use aspas simples ao redor do valor do hash, como mostrado. Aspas duplas ainda permitem que o shell expanda `$` antes que o docker o veja, corrompendo um hash argon2id real.
...
**Status das versões (16 de setembro de 2026):** O candidato v1.7 publicado é [v1.7.0-rc.16](https://github.com/CodesWhat/drydock/releases/tag/v1.7.0-rc.16). Este checkout acompanha o **desenvolvimento da v1.8, ainda não publicado**; os recursos da v1.8 no [roteiro](#roadmap) não estão incluídos nesse candidato. Os destaques de RC10 abaixo permanecem como notas históricas.
...
| 🔔  | **21 gatilhos**                            | 17 canais de notificação (Slack, Discord, Telegram, Teams, SMTP, MQTT, ntfy e mais) além de Docker, Docker Compose, Portainer e ações de comando, com modelos por evento/provedor, visualização ao vivo, filtragem de limite e modo em lote.                                                                                                                                                                                                                                                                                                         |
...
<tr><td>Canais de gatilho / notificação</td><td align="center">21</td><td align="center">17</td><td align="center">17</td><td align="center">~20</td></tr>
...
<tr><td>Autenticação OIDC / SSO</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Canais de gatilho / notificação</td><td align="center">21</td><td align="center">11+</td><td align="center">5</td><td align="center">15+</td></tr>
...
| **v1.7.0** | Atualizações inteligentes e UX | Ordenação de atualizações com reconhecimento de dependência ([#219](https://github.com/CodesWhat/drydock/discussions/219)), política de atualização por ação ([#511](https://github.com/CodesWhat/drydock/discussions/511)), relógio unificado de maturidade/idade de atualização, links de porta clicáveis, atalhos de teclado, PWA, ajuste de contraste do tema escuro (WCAG 2.2) ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865)), remoção de `DD_TRIGGER_*` (fim da janela de descontinuação da v1.5.0), curl removido da imagem |
| **v1.8.0** | Gestão de frota e configuração ao vivo | **Integrado em `dev/v1.8`, não publicado:** SQLite, fila de aprovação, configuração YAML, editores limitados de políticas de observadores/notificações/ações e modelos de resumos de segurança, atualizações paralelas e em massa seletivas ([#232](https://github.com/CodesWhat/drydock/discussions/232)), interface de dependências, inventário e limpeza de imagens, integridade da frota e atualização do inventário, agrupamento por rótulos compartilhável, janelas de manutenção apenas para instalações ([#946](https://github.com/CodesWhat/drydock/discussions/946)), autenticação própria, chaves API com escopo limitado ([#469](https://github.com/CodesWhat/drydock/discussions/469)) e progresso/dispositivos do Home Assistant ([#210](https://github.com/CodesWhat/drydock/discussions/210)). **Em revisão:** roteamento por grupo exato apenas para notificações ([#1251](https://github.com/CodesWhat/drydock/pull/1251)). **Pendente:** A configuração ao vivo ampliada e as políticas herdadas de grupos e ações precisam de decisões sobre quem controla os ajustes; TOTP precisa de um contrato aprovado; o observador de listas estáticas de imagens fica para depois da GA. |
| **v2.0+** | Expansão da plataforma e muito mais | Observadores Swarm/Kubernetes, sincronização GitOps, navegador de volume, imagens criadas localmente monitoradas em relação a uma base upstream declarada ([#897](https://github.com/CodesWhat/drydock/discussions/897)), gates de saúde, implementações canary, terminal web, RBAC, LDAP/AD, suporte básico ao Podman por meio de sua API compatível com Docker, CLI, imagem reforçada Wolfi, proxy de soquete |

**Requisitos pendentes da comunidade:** Atualizações e reinicializações de dependências entre hosts ([#219](https://github.com/CodesWhat/drydock/discussions/219)), maturidade baseada na contagem de versões ([#558](https://github.com/CodesWhat/drydock/discussions/558)) e uma configuração padrão de verificação de digests para toda a frota ([#657](https://github.com/CodesWhat/drydock/discussions/657)) ainda não foram implementados. A ordenação de dependências no mesmo host e sua nova interface não atendem ao pedido para hosts remotos. A detecção de builds locais e a escolha segura de uma ação de reconstrução continuam como trabalho futuro ([#897](https://github.com/CodesWhat/drydock/discussions/897)); nenhuma dessas entradas representa um compromisso de data de lançamento.
