...
| **v1.8.0** | Gerenciamento de frota e configuração ao vivo | Migração de armazenamento SQLite, fila de aprovação por atualização, configuração YAML, configuração de UI ao vivo, atualizações paralelas, atualizações seletivas em massa ([#232](https://github.com/CodesWhat/drydock/discussions/232)), UI de ordenação por dependência, gerenciador de imagens e remoção, observador de listas de imagens estáticas, janela de manutenção limitada a instalações por padrão ([#946](https://github.com/CodesWhat/drydock/discussions/946)), cadeia de autenticação própria substituindo o Passport.js (já lançada), autenticação de dois fatores TOTP, chaves de API com escopo, com limites de taxa por chave e revogação em cascata, para integrações HA/painel ([#469](https://github.com/CodesWhat/drydock/discussions/469)), progresso de atualização do Home Assistant + dispositivos por contêiner ([#210](https://github.com/CodesWhat/drydock/discussions/210)) |
