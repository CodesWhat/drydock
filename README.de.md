...
- **Ein Controller mit eigenem `local`-Watcher weist nicht mehr jeden Container ab, den ein Agent unter einem gleichnamigen Watcher meldet.** Die Eigentümerschaft eines Containers ohne Store-Eintrag wird jetzt danach entschieden, was die eigenen Watcher des Controllers tatsächlich aufgelistet haben, nicht nach einer Namensübereinstimmung. ([#1018](https://github.com/CodesWhat/drydock/pull/1018))
- **Ein Rollback eines Compose-verwalteten Containers stellt nicht mehr das Update wieder her, das es eigentlich rückgängig machen sollte.** Der Compose-Recreate reicht das übergebene Image jetzt an den Laufzeit-Refresh weiter, statt eines aus dem Update-Kandidaten des Containers abzuleiten; das automatische Rollback übergibt dem Health-Monitor den ganzen Container statt nur seiner ID und seines Namens, und das Backup-Image wird, digest-fixiert sofern vorhanden, gezogen, bevor irgendetwas gestoppt oder entfernt wird. ([#1023](https://github.com/CodesWhat/drydock/pull/1023), [#1029](https://github.com/CodesWhat/drydock/pull/1029))
- **Das veröffentlichte arm64-Image ist wieder ein echtes arm64-Image**, statt eines x86-64-Images mit arm64-Etikett. Beide Basis-Image-Pins verweisen wieder auf Multi-Arch-Image-Index-Digests, und der Release verweigert jetzt das Signieren, Taggen oder Promoten eines Images, dessen Binärdateien nicht zur veröffentlichten Plattform passen. ([#1024](https://github.com/CodesWhat/drydock/pull/1024))
- **Ein zu einem Agent verschobener Container bleibt nicht mehr hängen, wenn der lokale Watcher des Controllers abgeschaltet ist.** Die Startbereinigung, die veraltete Controller-Einträge entfernt, läuft jetzt auch, wenn nie ein lokaler Watcher konfiguriert war, sodass die Meldung des Agents nicht mehr als Controller-Eigentum abgewiesen wird. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Ein vom Controller an einen Agent übergebener Container kommt nicht mehr mit auf Standard zurückgesetztem Snooze, Reifemodus und übersprungenen Tags an.** Die Startbereinigung legt die Update-Richtlinie jetzt unter der Docker-ID des Containers ab, die der Umzug unangetastet lässt, statt unter einem Schlüssel, den der eingehende Eintrag nie hätte nachschlagen können. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Ein fehlgeschlagen registrierter lokaler Watcher löscht nicht mehr die Einträge seiner Container, wenn ein zweiter Watcher sich erfolgreich registriert.** Die Bereinigung behält jetzt jeden Eintrag, dessen Watcher noch konfiguriert ist, unabhängig davon, ob er in diesem Lauf registriert wurde, und wartet, bis jede Registrierung abgeschlossen ist, bevor sie die Registry liest. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Die Agents-Seite sagt jetzt, dass Registries auf jedem Agent konfiguriert werden müssen, nicht nur auf dem Controller**, mit einem durchgearbeiteten Gitea-Beispiel auf beiden Seiten. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))
- **Das Registry-Beispiel der Agents-Seite mountet jetzt die CA-Datei, auf die es verweist**, statt `DD_AGENT_REMOTE1_CAFILE` auf einen Pfad zu setzen, den der Compose-Service nie eingebunden hat. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Die Watchers-Seite sagt jetzt, dass das Wartungsfenster den gesamten geplanten Scan blockiert, nicht nur die Installation eines Updates.** Ein geschlossenes Fenster lässt neue Container unsichtbar, den Container-Status veraltet und Update-Benachrichtigungen aufgeschoben, bis es wieder öffnet; ein manueller Scan umgeht es weiterhin genauso wie ein manuelles Update. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))

Vollständige Release-Notes in [CHANGELOG.md](./CHANGELOG.md#170-rc11--2026-09-05).
...
| **v1.7.0** | Intelligente Updates und UX | Abhängigkeitsbewusste Aktualisierungsreihenfolge ([#219](https://github.com/CodesWhat/drydock/discussions/219)), Aktualisierungsrichtlinie pro Aktion ([#511](https://github.com/CodesWhat/drydock/discussions/511)), einheitliche Reife-/Update-Alters-Uhr, anklickbare Port-Links, Tastaturkürzel, PWA, Kontrastüberarbeitung im Dunkelmodus (WCAG 2.2) ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865)), Entfernung von `DD_TRIGGER_*` (Ende des Ablauffensters von Version 1.5.0), curl aus dem Image entfernt |
| **v1.8.0** | Flottenmanagement und Live-Konfiguration | SQLite-Store-Migration, Freigabe-Warteschlange pro Update, YAML-Konfiguration, Live-UI-Konfiguration, parallele Updates, selektive Massenaktualisierungen ([#232](https://github.com/CodesWhat/drydock/discussions/232)), Abhängigkeitsreihenfolge-UI, Bildmanager und -bereinigung, Watcher für statische Bildlisten, Wartungsfenster standardmäßig auf Installationen beschränkt ([#946](https://github.com/CodesWhat/drydock/discussions/946)), bereichsbezogene rotierbare API-Schlüssel (statische Bearer-Tokens für HA/Dashboard-Integrationen, [#469](https://github.com/CodesWhat/drydock/discussions/469)), TOTP-Zwei-Faktor-Anmeldung, Home Assistant-Updatefortschritt + Geräte pro Container ([#210](https://github.com/CodesWhat/drydock/discussions/210)) |
| **v2.0+** | Plattformerweiterung und darüber hinaus | Swarm/Kubernetes-Watcher, GitOps-Synchronisierung, Volume-Browser, lokal erstellte Images, die gegen eine deklarierte Upstream-Basis überwacht werden ([#897](https://github.com/CodesWhat/drydock/discussions/897)), Health Gates, Canary Deployments, Webterminal, RBAC, LDAP/AD, grundlegende Podman-Unterstützung über dessen Docker-kompatible API, CLI, gehärtetes Wolfi-Image, Socket-Proxy |
