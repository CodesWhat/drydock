...
- **Un contrôleur avec son propre watcher `local` ne rejette plus tous les conteneurs qu'un agent signale sous un watcher du même nom.** La propriété d'un conteneur sans ligne dans le store est désormais décidée d'après ce que les propres watchers du contrôleur ont réellement énuméré, et non par une coïncidence de nom. ([#1018](https://github.com/CodesWhat/drydock/pull/1018))
- **Une restauration d'un conteneur géré par Compose ne redéploie plus la mise à jour qu'elle était censée annuler.** La recréation Compose transmet désormais l'image reçue au rafraîchissement à l'exécution au lieu d'en dériver une du candidat de mise à jour du conteneur ; la restauration automatique transmet au moniteur de santé le conteneur entier au lieu de son seul id et nom, et l'image de sauvegarde est récupérée, épinglée par digest quand l'enregistrement en porte un, avant que quoi que ce soit ne soit arrêté ou supprimé. ([#1023](https://github.com/CodesWhat/drydock/pull/1023), [#1029](https://github.com/CodesWhat/drydock/pull/1029))
- **L'image arm64 publiée est de nouveau une véritable image arm64**, et non une image x86-64 portant l'étiquette arm64. Les deux épinglages d'image de base pointent de nouveau vers des digests d'index d'image multi-architecture, et la version refuse désormais de signer, taguer ou promouvoir une image dont les binaires ne correspondent pas à la plateforme sous laquelle elle est publiée. ([#1024](https://github.com/CodesWhat/drydock/pull/1024))
- **Un conteneur déplacé vers un agent ne reste plus bloqué quand le watcher local du contrôleur est désactivé.** L'élagage au démarrage qui nettoie les enregistrements obsolètes appartenant au contrôleur s'exécute désormais aussi quand aucun watcher local n'a jamais été configuré, si bien que le rapport de l'agent n'est plus rejeté comme appartenant au contrôleur. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Un conteneur transféré du contrôleur à un agent n'arrive plus avec son snooze, son mode de maturité et ses tags ignorés réinitialisés aux valeurs par défaut.** L'élagage au démarrage range désormais la politique de mise à jour sous l'id Docker du conteneur, que le déplacement laisse intact, au lieu d'une clé que l'enregistrement entrant n'aurait jamais pu retrouver. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Un watcher local qui échoue à s'enregistrer ne supprime plus les enregistrements de ses conteneurs quand un second watcher s'enregistre correctement.** L'élagage conserve désormais tout enregistrement dont le watcher est encore configuré, qu'il se soit enregistré ou non lors de cette exécution, et attend que chaque enregistrement se termine avant de lire le registre. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **La page des agents indique désormais que les registres doivent être configurés sur chaque agent, pas seulement sur le contrôleur**, avec un exemple Gitea complet des deux côtés. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))
- **L'exemple de registres de la page des agents monte désormais le fichier CA qu'il référence**, au lieu de faire pointer `DD_AGENT_REMOTE1_CAFILE` vers un chemin que le service Compose n'a jamais monté. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **La page des watchers indique désormais que la fenêtre de maintenance bloque tout le scan planifié, pas seulement l'installation d'une mise à jour.** Une fenêtre fermée laisse les nouveaux conteneurs invisibles, l'état des conteneurs obsolète et les notifications de mise à jour différées jusqu'à sa réouverture ; un scan manuel continue de la contourner, comme une mise à jour manuelle. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))

Notes complètes dans [CHANGELOG.md](./CHANGELOG.md#170-rc11--2026-09-05).
...
- **L'authentification de pull des registries, les fuites dans les réponses d'erreur et le masquage des erreurs de prévisualisation sont tous renforcés** : treize registries (Hub, Custom, DHI, DOCR, Harbor, Gitea, Forgejo, Codeberg, Nexus, Artifactory, Alibaba CR, OCIR, IBM CR) s'authentifiaient pour la vérification de version puis effectuaient le pull de façon anonyme, car le constructeur d'identifiants de pull n'avait aucune branche pour une valeur `auth` configurée ; il décode désormais cette valeur de la même manière que le fait déjà le constructeur d'identifiants de recherche, et une valeur malformée échoue désormais de façon fermée au lieu de ne rien renvoyer silencieusement. Huit gestionnaires de l'API n'interpolent plus un message d'erreur brut – qui pouvait contenir un en-tête `Authorization` ou une URL de webhook avec des identifiants – dans une réponse 500, et les acheminent désormais via le nettoyeur `sanitizePreviewErrorReason` déjà existant, qui redacte désormais aussi les identifiants intégrés dans un segment de chemin d'URL (URL de webhook Telegram, IFTTT et Discord), pas seulement dans les en-têtes ou les informations utilisateur.
- **La validation des paramètres de requête est désormais cohérente sur les points de terminaison log, agent et audit** : un `tail` ou `since` non numérique plaçait auparavant `NaN` dans la lecture du tampon circulaire au lieu d'être rejeté, un `?tail=` vide était lu comme absent plutôt qu'invalide, et un `limit`/`offset` avec un préfixe numérique comme `?limit=25logs` était validé sur ses chiffres de tête au lieu d'échouer ; les trois rejettent désormais tout ce qui n'est pas un entier propre et complet.
- **Six défauts d'interface sont corrigés** : la sélection de ligne ne mettait jamais réellement en surbrillance dans sept vues, car le tableau de données partagé déclare `selectedKey`, mais chaque vue lui passait `active-row` à la place ; le texte blanc avec un contraste aussi bas que 1,37:1 sur le bouton de test des triggers et sur deux avatars est aplati vers un token qui atteint 4,5:1 dans les douze thèmes ; la boîte de sortie des notifications et la vue détaillée plein écran d'un conteneur avaient chacune leur propre situation de concurrence où la vue s'affichait avant que ses données ne soient résolues, les deux sont désormais protégées ; deux watchers du tableau de bord manquaient chaque mise à jour SSE en place car ils observaient une simple ref plutôt qu'une source sensible à la longueur ou à l'empreinte ; et le texte de statut qui s'affichait sous forme de valeurs d'énumération anglaises brutes à cinq endroits est désormais traduit dans les 16 langues.
- **2109 chaînes qui affichaient encore le texte source en anglais sont désormais réellement traduites**, dans les 16 langues non anglaises : une grande partie de la liste des conteneurs, des boîtes de dialogue de mise à jour et de restauration, de la palette de recherche et de la boîte de sortie des notifications retombait silencieusement en anglais quelle que soit la langue sélectionnée. Le workflow de synchronisation hebdomadaire Crowdin ne réinitialise plus non plus les six README traduits vers l'anglais : `README.md` n'est plus enregistré comme source Crowdin, et les README traduits sont désormais rédigés à la main dans le dépôt et vérifiés phrase par phrase à chaque coupure de version. ([#919](https://github.com/CodesWhat/drydock/pull/919))
- **Corrections de fiabilité pour la release et la CI** : la compilation de fumée multi-architecture réessaie désormais face à une situation de concurrence ouverte dans BuildKit (moby/buildkit#7089) qui pouvait préfixer deux fois le chemin de l'émulateur QEMU et faire échouer purement et simplement une compilation multi-architecture, et la coupure de release elle-même bénéficie désormais d'une nouvelle tentative de compilation complète pour le cas où la première tentative n'aurait produit aucun digest ; le scan DAST hebdomadaire, qui ne s'était jamais terminé car ZAP consommait à lui seul 39m46s du budget de 40 minutes et privait Nuclei de ressources, exécute désormais les deux scanners comme des jobs parallèles séparés ; et la recherche de documentation, qui renvoyait auparavant environ 1600 résultats répartis sur cinq versions archivées avec le changelog le plus ancien classé en premier, se limite désormais à la version en cours de lecture.

Notes complètes dans [CHANGELOG.md](./CHANGELOG.md#170-rc6--2026-08-29).

</details>

<details>
<summary><strong>Points forts de la v1.7.0-rc.5</strong></summary>

- **Un durcissement de sécurité corrige cinq constats dans Portwing et sur la surface de débogage/diagnostic** : une charge utile hello de Portwing malformée est désormais validée avant d'être analysée, au lieu de lever une exception hors de la limite d'erreur du callback ; la propriété des conteneurs de l'agent est désormais appliquée à la limite de mise à jour/suppression ; le masquage détecte désormais aussi les valeurs `*_PAT` et les identifiants intégrés dans des URL (y compris relatives au schéma) ; et le chemin de diagnostic d'origine rejetée est désormais limité en débit. ([#904](https://github.com/CodesWhat/drydock/pull/904))
- **Les thèmes sombres respectent désormais le contraste minimum WCAG 2.2** : le texte secondaire/atténué, les couleurs de ton, les surfaces des toasts et les libellés des boutons principaux sont relevés pour atteindre 4,5:1 par rapport aux surfaces sur lesquelles ils sont réellement peints, sur les six thèmes sombres. ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865))
- **Les grandes flottes et les clients lents ne cassent plus la connexion au contrôleur** : un agent dont le replay de watcher mis en cache dépassait 256 Ko ne pouvait plus jamais se reconnecter ; le flux reste désormais ouvert pour que le handshake authentifié fournisse l'état ; les clients SSE en retard reçoivent désormais une livraison bornée, consciente du drain et dans l'ordre, au lieu d'écritures abandonnées ou d'une mémoire illimitée ; le limiteur du journal système ne se rabat plus sur une identité vide ; et un transport d'agent non pris en charge est désormais rejeté dès l'admission au lieu d'échouer plus tard. ([#904](https://github.com/CodesWhat/drydock/pull/904))
- **L'état du cycle de vie des mises à jour et des watchers reste exact à travers les redémarrages et les démontages** : la récupération au démarrage ne marque plus un conteneur inchangé comme mis à jour, un redémarrage ne supprime plus les événements de fin de lot pour les mises à jour encore en cours, une mise à jour qui n'a jamais démarré n'est plus signalée comme un échec, un watcher démonté en cours de configuration ne peut plus être ressuscité par un callback tardif, et les segments d'événements Docker analysés simultanément ne se disputent plus un tampon partagé. ([#904](https://github.com/CodesWhat/drydock/pull/904))
- **Corrections de cohérence pour les sauvegardes, les restaurations et les listes de conteneurs** : les sauvegardes portent désormais une identité stable et cantonnée au lieu d'entrer en collision sur un nom de conteneur partagé ; une restauration récupère désormais le digest enregistré avec la sauvegarde plutôt que ce que pointe un tag mutable à cet instant ; les analyses de digest concurrentes ne s'annulent plus mutuellement ; une action de conteneur réussie ne renvoie plus 500 lorsque le rafraîchissement qui suit échoue ; et les listes de conteneurs paginées sont désormais triées globalement et non plus seulement au sein d'une page. ([#904](https://github.com/CodesWhat/drydock/pull/904))
- **Le workflow de synchronisation Crowdin n'échoue plus sur les branches dev non par défaut** : un push vers une branche `dev/vX.Y` qui n'était pas la plus récente se terminait par un conflit de checkout car le résolveur de base choisissait toujours la branche dev la plus élevée, quelle que soit la référence ayant déclenché l'exécution ; un push cible désormais directement sa propre branche. ([run 33047712284](https://github.com/CodesWhat/drydock/actions/runs/33047712284))

Notes complètes dans [CHANGELOG.md](./CHANGELOG.md#170-rc5--2026-08-27).
...
- **Les flux de logs WebSocket fonctionnent désormais derrière des proxys à terminaison TLS** : avec le trust proxy activé et `X-Forwarded-Proto` absent de la requête d'upgrade, la vérification d'origine ne se rabat plus sur l'état TLS du socket local (HTTP en clair derrière la terminaison TLS, ce qui faisait échouer chaque connexion navigateur avec un 403) ; le protocole est désormais traité comme inconnu et la validation de l'hôte reste inchangée. Traefik transmet le schéma côté client de l'upgrade sous la forme `wss` plutôt que `https` (traefik/traefik#6388), ce que la vérification d'origine rejetait purement et simplement, de sorte que le premier correctif seul échouait encore avec un 403 derrière une configuration Traefik par défaut ; `ws`/`wss` sont désormais mappés vers `http:`/`https:` pour la comparaison d'origine. ([#867](https://github.com/CodesWhat/drydock/issues/867), [#868](https://github.com/CodesWhat/drydock/pull/868), [#887](https://github.com/CodesWhat/drydock/pull/887))
- **Le démarrage ne plante plus lorsque le volume du store refuse `chmod`** : le durcissement des permissions introduit en 1.6.0 levait une exception sur `EPERM`, si bien que les montages refusant `chmod` (volumes NFS/CIFS, conteneurs non-root) faisaient planter tout le processus au démarrage et bloquaient purement et simplement les mises à niveau depuis 1.6.0 ; il se contente désormais d'avertir et de continuer sur `EPERM`/`EACCES`/`ENOTSUP` ; un volume réellement en lecture seule (`EROFS`) échoue quand même rapidement au démarrage, car rien ne pourrait de toute façon y être persisté. ([#874](https://github.com/CodesWhat/drydock/discussions/874), [#886](https://github.com/CodesWhat/drydock/pull/886))
- **Le dump de débogage masquait les noms des variables d'environnement au lieu de leurs valeurs** : les entrées d'environnement sont des paires `{key, value}`, et le parcours de masquage comparait le nom littéral de la propriété `key` à sa règle de jetons sensibles, si bien qu'une variable comme `HF_TOKEN` ressortait avec le nom masqué et le secret en clair ; les noms restent désormais visibles et les valeurs sont masquées lorsque le nom correspond à une règle sensible. ([#875](https://github.com/CodesWhat/drydock/issues/875), [#885](https://github.com/CodesWhat/drydock/pull/885))
...
- **Les conteneurs qui sortent du périmètre de surveillance sont désormais purgés du store et de l'interface** : un conteneur exclu parce que `watchbydefault` est désactivé, ou parce que son label `dd.watch` a été retiré, conservait un enregistrement obsolète tant qu'il restait inspectable dans Docker ; les conteneurs arrêtés mais toujours surveillés conservent leur comportement habituel du bouton de démarrage. ([#869](https://github.com/CodesWhat/drydock/issues/869), [#888](https://github.com/CodesWhat/drydock/pull/888))
...
> ✅ = pris en charge &nbsp; ❌ = non pris en charge &nbsp; ⚠️ = partiel / limité &nbsp; ? = non confirmé &nbsp; † = archivé, n'est plus conservé

<h4 align="center">Gestionnaires de mises à jour</h4>

<table>
<thead>
<tr>
<th width="32%">Fonctionnalité</th>
...
<tbody>
<tr><td>Maintenance active</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td></tr>
<tr><td>Interface web / tableau de bord</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>Mise à jour automatique des conteneurs</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td><td align="center">✅</td></tr>
<tr><td>Mises à jour Docker Compose</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td><td align="center">⚠️</td></tr>
<tr><td>Mises à jour compatibles SemVer</td><td align="center">✅</td><td align="center">✅</td><td align="center">⚠️</td><td align="center">❌</td></tr>
...
<tr><td>Journal d'audit</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>RBAC / rôles multi-utilisateurs</td><td align="center">❌</td><td align="center">❌</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>Authentification OIDC / SSO</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>Canaux de déclenchement / notification</td><td align="center">20</td><td align="center">17</td><td align="center">17</td><td align="center">~20</td></tr>
<tr><td>MQTT / Home Assistant</td><td align="center">✅</td><td align="center">✅</td><td align="center">⚠️</td><td align="center">❌</td></tr>
<tr><td>Fournisseurs de registres</td><td align="center">23</td><td align="center">12</td><td align="center">⚠️</td><td align="center">⚠️</td></tr>
<tr><td>API REST</td><td align="center">✅</td><td align="center">✅</td><td align="center">⚠️</td><td align="center">⚠️</td></tr>
<tr><td>API webhook pour CI/CD</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">✅</td></tr>
<tr><td>Métriques Prometheus</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Agents distribués (distants)</td><td align="center">✅</td><td align="center">⚠️</td><td align="center">❌</td><td align="center">⚠️</td></tr>
<tr><td>Regroupement de conteneurs / piles</td><td align="center">✅</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>Démarrer/arrêter/redémarrer/mettre à jour</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">❌</td></tr>
<tr><td>Visionneuse de journaux de conteneur</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">❌</td></tr>
</tbody>
</table>

> Watchtower a été archivé en décembre 2025 et sa dernière version était la v1.7.1 (novembre 2023). Un fork communautaire non officiel, nicholas-fedor/watchtower, continue de publier des versions.

<h4 align="center">Plateformes de gestion</h4>
...
<tr><td>Interface web / tableau de bord</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Mise à jour automatique des conteneurs</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Mises à jour Docker Compose</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Mises à jour compatibles SemVer</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">✅</td></tr>
<tr><td>Surveillance des digests</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
...
<tr><td>RBAC / rôles multi-utilisateurs</td><td align="center">❌</td><td align="center">✅</td><td align="center">✅</td><td align="center">⚠️</td></tr>
<tr><td>Authentification OIDC / SSO</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Canaux de déclenchement / notification</td><td align="center">20</td><td align="center">11+</td><td align="center">5</td><td align="center">15+</td></tr>
<tr><td>MQTT / Home Assistant</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">✅</td></tr>
<tr><td>Fournisseurs de registres</td><td align="center">23</td><td align="center">⚠️</td><td align="center">⚠️</td><td align="center">⚠️</td></tr>
<tr><td>Métriques Prometheus</td><td align="center">✅</td><td align="center">❌</td><td align="center">❌</td><td align="center">✅</td></tr>
<tr><td>Agents distribués (distants)</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Regroupement de conteneurs / piles</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">?</td></tr>
</tbody>
</table>

> Compilé à partir de la documentation et des dépôts publics de chaque projet, le 29/08/2026.
...
| **v1.7.0** | Mises à jour intelligentes et UX | Ordre de mise à jour tenant compte des dépendances ([#219](https://github.com/CodesWhat/drydock/discussions/219)), politique de mise à jour par action ([#511](https://github.com/CodesWhat/drydock/discussions/511)), horloge unifiée de maturité/âge des mises à jour, liens de port cliquables, raccourcis clavier, PWA, amélioration du contraste du thème sombre (WCAG 2.2) ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865)), suppression de `DD_TRIGGER_*` (fin de la fenêtre de dépréciation de la v1.5.0), curl supprimé de l'image |
| **v1.8.0** | Gestion de flotte et configuration en direct | Migration du magasin SQLite, file d'attente d'approbation par mise à jour, configuration YAML, configuration de l'interface utilisateur en direct, mises à jour parallèles, mises à jour sélectives en masse ([#232](https://github.com/CodesWhat/drydock/discussions/232)), UI d'ordonnancement des dépendances, gestionnaire d'images et élagage, observateur de listes d'images statiques, fenêtre de maintenance limitée aux installations par défaut ([#946](https://github.com/CodesWhat/drydock/discussions/946)), clés API rotatives à portée limitée (jetons Bearer statiques pour les intégrations HA/tableau de bord, [#469](https://github.com/CodesWhat/drydock/discussions/469)), connexion à deux facteurs TOTP, progression des mises à jour Home Assistant + appareils par conteneur ([#210](https://github.com/CodesWhat/drydock/discussions/210)) |
| **v2.0+** | Extension de la plateforme et au-delà | Observateurs Swarm/Kubernetes, synchronisation GitOps, navigateur de volumes, images construites localement surveillées par rapport à une base upstream déclarée ([#897](https://github.com/CodesWhat/drydock/discussions/897)), barrières de santé, déploiements Canary, terminal Web, RBAC, LDAP/AD, prise en charge basique de Podman via son API compatible Docker, CLI, image renforcée Wolfi, proxy de socket |
