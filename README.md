...
| **v1.4.1** | Reliability & Resilience | API error sanitization, agent log validation, TLS path redaction, audit store indexing with 30-day retention, backup store indexed lookups, type-safe store modules, durable batch scans across navigation, recent-status API endpoint, self-update controller testable entrypoint |
