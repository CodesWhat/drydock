...
| **v1.8.0** | Fleet Management & Live Config | SQLite store migration, per-update approval queue (shipped), YAML config (shipped), live UI config, parallel updates, selective bulk updates ([#232](https://github.com/CodesWhat/drydock/discussions/232)), dependency-ordering UI, images manager and prune, static image list watcher, maintenance window scoped to installs by default ([#946](https://github.com/CodesWhat/drydock/discussions/946)), first-party auth chain replacing Passport.js (shipped), TOTP two-factor auth, scoped API keys with per-key rate limits and cascading revocation, for HA/dashboard integrations ([#469](https://github.com/CodesWhat/drydock/discussions/469)), Home Assistant update progress + per-container devices ([#210](https://github.com/CodesWhat/drydock/discussions/210)) |
