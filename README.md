...
| **v1.4.1** | Reliability & Resilience | API error sanitization, trigger body + HTTP auth schema validation, identity-aware rate limiter keying, PUT deprecation with RFC headers, CORS wildcard deprecation warning, OpenAPI action-endpoint convention, backup pruning on failure + undefined guard, rollback audit accuracy, reactive server feature flags, compose trigger hardening (auto-detection, validation, reconciliation, digest pinning, batch mode, multi-file awareness, compose path UI), advisory-only security scanning (`NONE` severity) |
