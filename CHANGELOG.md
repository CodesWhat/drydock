...
- **Retired the self-hosted star-history provider and its homepage card.** Warpchart is the house standard for growth charts, so maintaining a second first-party implementation of the same thing was duplicated infrastructure. `/api/star-history` had also been quietly serving its placeholder "check back shortly" SVG rather than a real chart — and because it does so at HTTP 200, nothing ever went red. Deleted the route handler, its SVG-rendering and request-resolution libraries, and the homepage `StarHistory` card. The README chart renders through Warpchart instead.
...
- **Removed `curl` from the Docker image.** Deprecated since v1.5.x with removal scheduled for v1.7.0. The image's own healthcheck has used the compiled `/bin/healthcheck` binary since v1.5.0; user-defined HEALTHCHECK overrides that shell out to curl will stop working and should switch to `test: /bin/healthcheck ${DD_SERVER_PORT:-3000}` or drop the override. Drydock now also logs a startup warning naming the container when it detects that its own healthcheck override still shells out to curl (in addition to the existing UI banner).
