...
- **`DD_SESSION_SECRET` docs corrected to reflect rc.20 behavior (doc fix).** rc.20 removed the auto-generated session-secret fallback (`app/api/auth-session.ts`) — the server now refuses to start without `DD_SESSION_SECRET` set. The `content/docs/current/configuration/server/index.mdx` table still listed the variable as optional with `auto-generated` as the default. rc.21 corrects the row: Required indicator changed from ⚪ to 🔴, description updated to state the server refuses to start if unset, and the recommended value format (`openssl rand -hex 32`) added. No behavior change — this is a doc correction only.
