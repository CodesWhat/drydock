...
- **Lookout/1.0 edge WebSocket endpoint and Ed25519 agent-key registry (PR #429, M5).** `lookout` agents behind NAT or a firewall can now dial out to drydock over WSS instead of waiting for an inbound connection. The WebSocket endpoint is at `/api/lookout/ws` (alias `/api/v1/lookout/ws`) using the `lookout/1.0` subprotocol; authentication is by Ed25519 public key with timestamp + nonce replay protection (±60 s clock-skew window, 16 MB max frame). Operator key management is available via the REST API at `/api/lookout/keys` (list, register, revoke).
