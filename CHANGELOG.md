...
- **Audit nav icon no longer renders blank for the Lucide icon-library preference.** PR [#668](https://github.com/CodesWhat/drydock/pull/668)'s icon-bundle regeneration against tabler 1.2.38 dropped `lucide:history` from `ui/src/boot/icon-bundle.json` because the installed `@iconify-json/lucide` (1.2.121) no longer ships a `history` glyph, leaving `ui/src/icons.ts`'s `iconMap.audit.lucide` pointing at a key the bundle no longer had. Repointed to `lucide:rotate-ccw-clock` (the same clock-with-counter-clockwise-arrow concept `fa6-solid:clock-rotate-left`/`ph:clock-counter-clockwise` already use for the same entry). A bundle-consistency sweep against the currently installed iconsets turned up the same class of stale reference for `iconoir:history`, `lucide:more-vertical`, `iconoir:key-alt`, and `iconoir:gitlab` (all renamed upstream) plus four `fa6-brands:*` icons that were never bundled because `@iconify-json/fa6-brands` was missing from `ui/package.json` entirely — all fixed or backfilled so every `<library>:<icon>` reference in `iconMap` now resolves. A new `ui/tests/icons.spec.ts` asserts every `iconMap` entry has a matching `icon-bundle.json` key so this class of regression fails CI instead of shipping silently.
