...
- **An empty `tail` or `since` query value on the agent log endpoint was treated as absent, not invalid.** `getValidatedLogInteger` guarded with `if (!value) return undefined`, and `''` is falsy along with `null` and `undefined`, so `?tail=` and `?since=` skipped the 400 introduced for non-numeric values and silently passed `undefined` through to `getEntries` instead of being rejected as malformed input. The guard now only treats `undefined` and `null` as absent; an empty string proceeds to the integer-shape check like any other supplied value and is rejected.
