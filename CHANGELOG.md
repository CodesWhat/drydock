...
- **Bulk container ingestion had no ownership check at all.** The gate landed for #904 only covers the two incremental `dd:container-*` event handlers. Handshake, the watcher-snapshot fallback, on-demand `watch`/`watchContainer`, and edge `handleContainerSync` all reach `buildContainerReport` through `processAuthoritativeContainer` with no ownership check in between, so a connected agent could name a container id already owned by another agent or by the controller's own local watcher and have the store reassign it, repeatedly, at its own cadence. `processAuthoritativeContainer` now runs a no-existing-record and different-owner check before ingesting. It deliberately skips a `watcher` mismatch check against the stored record, because bulk ingestion is exactly how a legitimate watcher rename propagates (an operator renaming a `DD_WATCHER_<NAME>_SOCKET` key), and rejecting on that mismatch here would silently stop every future report for that container id from landing, with nothing left to prune or retry it. Ported from the v1.7 line. ([#922](https://github.com/CodesWhat/drydock/pull/922))
