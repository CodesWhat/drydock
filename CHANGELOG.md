...
- **An empty `tail` or `since` on the log and agent endpoints (`?tail=`) was read as absent rather than invalid.** The validator's guard was `if (!value) return undefined`, and an empty string is falsy along with `null` and `undefined`, so a supplied-but-empty value skipped the 400 above it and was silently forwarded downstream as `undefined` instead of being rejected. The guard now only treats a genuinely missing parameter as absent.
