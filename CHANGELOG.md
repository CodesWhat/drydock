...
- **Cancel queued updates ([commits `4b79e3ac`, `79487115`](https://github.com/CodesWhat/drydock/commits/feature/v1.5-rc17)).** New `POST /api/operations/:id/cancel` endpoint marks a queued update operation as failed with `lastError: 'Cancelled by operator'`. In-progress operations return `409 Conflict` — mid-flight cancellation with auto-rollback requires lifecycle checkpoints in `UpdateLifecycleExecutor` and is intentionally deferred to a future release. The container row gains a Cancel action visible only when the row's `updateOperation.status === 'queued'`; clicking it hits the new endpoint and surfaces success/conflict/missing toasts.
