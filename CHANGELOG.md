...
- **Deprecation warnings, banners, and docs corrected to match what the code actually does.** A lifecycle audit of every deprecated feature found several places where the signal didn't match reality: two deprecation banners (HTTP OIDC discovery, `curl` healthcheck overrides) told users to set env vars that don't exist anywhere in the codebase; the unversioned `/api/*` alias's UI deprecation banner could never fire because the backend never reported alias usage; four `wud.*` label fallbacks (`wud.group`, `wud.webhook.enabled`, `wud.display.picture`, `wud.compose.file`) worked silently with no warning at all; the registry `PUBLIC_TOKEN` migration guidance pointed at env vars missing a required instance-name segment, so following it didn't actually authenticate; a `Sunset` response header's hardcoded date carried an internally-inconsistent day-of-week; and the Home Assistant MQTT multi-agent topic-layout change had no startup warning or deprecation-schedule entry. `DEPRECATIONS.md` and the published deprecation schedule were also corrected where they cited a stale removal version (the CORS implicit-wildcard fallback and the legacy aggregate-stats endpoint were both already removed, months before the versions the docs claimed) or described behavior the code no longer has (CORS, `WATCHATSTART`). None of this changes runtime behavior beyond adding the missing warnings — it's the signals catching up to the code.
