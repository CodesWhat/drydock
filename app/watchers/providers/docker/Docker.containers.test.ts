...
describe('Docker Watcher', () => {
  ...
  beforeEach(async () => {
    ...
    // assert getContainers() enriches/returns freshly-discovered containers
    // synchronously within a single call and predate the settling window.
  ...
  describe('Additional Coverage - filterByCurrentPrefix', () => {
    test('should warn when no tags match prefix', async () => {
      ...
      registry.getState.mockReturnValue({
        registry: {
          hub: { normalizeImage: (img) => img, getTags: vi.fn().mockResolvedValue(['2.0.0']) },
        },
      });
      const logChild = { error: vi.fn(), warn: vi.fn(), debug: vi.fn() };
      await docker.findNewVersion(container, logChild);
    ...
    test('should return all tags when current tag has no numeric part', async () => {
      ...
      registry.getState.mockReturnValue({
        registry: {
          hub: {
            normalizeImage: (img) => img,
            getTags: vi.fn().mockResolvedValue(['latest', 'stable', '1.0.0']),
          },
        },
      });
