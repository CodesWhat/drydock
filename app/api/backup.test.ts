...
describe('Backup Router', () => {
  ...
  describe('rollbackContainer', () => {
    ...
    test('should reject a legacy backup that carries no identity key', async () => {
      const handler = getHandler('post', '/:id/rollback');
      const target = {
        id: 'a1',
        name: 'web',
    ...
    test('should pull the backup image exactly once and never through the rollback pull helper', async () => {
      ...
      expect(mockTrigger.pullImage).toHaveBeenCalledWith(
        {},
        {},
        'library/nginx:1.24@sha256:kept',
        expect.anything(),
      );
    ...
    test('deploys the backup image, not the update candidate, through the REAL Dockercompose.recreateContainer', async () => {
      ...
      expect(createContainerSpy).toHaveBeenCalledWith(
        dockerApi,
        expect.objectContaining({ Image: 'library/nginx:1.24@sha256:kept' }),
        'nginx',
        expect.anything(),
      );
      expect(res.status).toHaveBeenCalledWith(200);
