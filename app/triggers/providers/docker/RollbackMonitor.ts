...
type RollbackContainer = {
  name: string;
  // Required for the same reason `image.registry` below is: the health
  // monitor's rollback runs through the trigger's own recreate, and the compose
  // action resolves the watcher from this container (DR-101).
  watcher: string;
  agent?: string;
