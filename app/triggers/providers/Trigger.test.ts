...
describe('notification group routing', () => {
  const groupedContainer = (labels: Record<string, string> = { 'dd.group': 'payments' }) =>
    ({
      id: 'grouped-container',
      name: 'payments-api',
      watcher: 'local',
      labels,
      updateAvailable: true,
      updateKind: { kind: 'tag', localValue: '1.0', remoteValue: '2.0', semverDiff: 'major' },
    }) as Parameters<Trigger['mustTrigger']>[0];

  beforeEach(() => {
    trigger.type = 'slack';
    trigger.name = 'grouped';
    trigger.configuration = { ...configurationValid, group: 'payments' };
  ...
  test.each(['payments', 'Payments', ' literal [a.*] ', '__ungrouped__'])(
    ...
    (group) => {
      expect(trigger.validateConfiguration({ ...configurationValid, group }).group).toBe(group);
    },
  );

  test.each(['', '   ', '\n', null, 42, ['payments']])('rejects invalid group %j', (group) => {
    expect(() => trigger.validateConfiguration({ ...configurationValid, group } as any)).toThrow();
  });

  test.each(['docker', 'dockercompose', 'portainer', 'command'])(
    'rejects group configuration for the %s action',
    (type) => {
      trigger.type = type;
      expect(() => trigger.validateConfiguration(trigger.configuration)).toThrow();
    },
  );

  test.each([
    [{ 'dd.group': 'payments' }, true],
    [{ 'dd.group': 'Payments' }, false],
    [{ 'dd.group': 'media' }, false],
    [{ 'com.docker.compose.project': 'payments' }, true],
    [{ 'com.docker.stack.namespace': 'payments' }, true],
    [{ 'dd.group': 'media', 'com.docker.compose.project': 'payments' }, false],
    [{ 'com.docker.compose.project': 'media', 'com.docker.stack.namespace': 'payments' }, false],
    [{ 'dd.group': '', 'com.docker.compose.project': 'payments' }, false],
    [{}, false],
  ])('matches server label priority %j => %s', (labels, expected) => {
    expect(trigger.mustTrigger(groupedContainer(labels))).toBe(expected);
  });

  test('treats special characters literally and never matches the ungrouped bucket', () => {
    trigger.configuration.group = 'pay.*';
    expect(trigger.mustTrigger(groupedContainer())).toBe(false);
    expect(trigger.mustTrigger(groupedContainer({ 'dd.group': 'pay.*' }))).toBe(true);
    trigger.configuration.group = '__ungrouped__';
    expect(trigger.mustTrigger(groupedContainer({}))).toBe(false);
    expect(trigger.mustTrigger(groupedContainer({ 'dd.group': '__ungrouped__' }))).toBe(true);
  });

  test('preserves exclusions and agent affinity for matching groups', () => {
    const container = groupedContainer();
    expect(trigger.mustTrigger({ ...container, notificationTriggerExclude: 'slack.grouped' })).toBe(
      false,
    );
    expect(trigger.mustTrigger({ ...container, agent: 'remote' })).toBe(true);
    trigger.agent = 'other';
    expect(trigger.mustTrigger({ ...container, agent: 'remote' })).toBe(false);
    expect(trigger.mustTrigger({ ...container, agent: 'other' })).toBe(true);
    trigger.agent = undefined;
    trigger.strictAgentMatch = true;
    expect(trigger.mustTrigger({ ...container, agent: 'remote' })).toBe(false);
  });

  test('keeps unfiltered notifications compatible', () => {
    delete trigger.configuration.group;
    expect(trigger.mustTrigger(groupedContainer({}))).toBe(true);
    expect(trigger.mustTrigger(groupedContainer({ 'dd.group': 'media' }))).toBe(true);
  });

  test.each(['simple', 'batch', 'digest'])('filters real %s update dispatch', async (mode) => {
    trigger.configuration.mode = mode;
    const send = vi.spyOn(trigger, 'trigger').mockResolvedValue(undefined);
    const sendBatch = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
    const matching = groupedContainer();
    const other = { ...groupedContainer({ 'dd.group': 'media' }), id: 'other', name: 'other' };
    const reports = [matching, other].map((container) => ({ container, changed: true }));
    if (mode === 'simple') {
      for (const report of reports) await trigger.handleContainerReport(report);
      expect(send.mock.calls.map(([container]) => container.id)).toEqual([matching.id]);
    } else {
      if (mode === 'batch') await trigger.handleContainerReports(reports);
      else {
        for (const report of reports) await trigger.handleContainerReportDigest(report);
        await trigger.flushDigestBuffer();
      }
      expect(sendBatch).toHaveBeenCalledTimes(1);
      expect(sendBatch.mock.calls[0][0].map((container) => container.id)).toEqual([matching.id]);
    }
  });

  test('filters lifecycle notifications and no-group agent events', async () => {
    const send = vi.spyOn(trigger, 'trigger').mockResolvedValue(undefined);
    for (const group of ['payments', 'media']) {
      await trigger.handleContainerUpdateFailedEvent({
        containerName: group,
        container: groupedContainer({ 'dd.group': group }),
        error: 'update failed',
      });
    }
    await trigger.handleAgentDisconnectedEvent({ agentName: 'edge', reason: 'offline' });
    await trigger.handleAgentConnectedEvent({ agentName: 'edge', reconnected: true });
    await Promise.resolve();
    expect(send).toHaveBeenCalledTimes(1);
  });

  test.each(['simple', 'digest', 'batch+digest'])(
    'filters %s security alerts',
    async (securitymode) => {
      trigger.configuration.securitymode = securitymode;
      const send = vi.spyOn(trigger, 'trigger').mockResolvedValue(undefined);
      const sendBatch = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
      for (const group of ['payments', 'media']) {
        await trigger.handleSecurityAlertEvent({
          containerName: group,
          container: { ...groupedContainer({ 'dd.group': group }), id: group, name: group },
          cycleId: 'group-cycle',
      ...
      await Promise.resolve();
      if (securitymode === 'simple') expect(send).toHaveBeenCalledTimes(1);
      else {
        expect(sendBatch).toHaveBeenCalledTimes(1);
        expect(sendBatch.mock.calls[0][0]).toHaveLength(1);
        expect(sendBatch.mock.calls[0][0][0].name).toContain('payments');
      }
    },
  );

  test.each(['update', 'security'])(
    'revalidates group membership before %s digest delivery',
    async (kind) => {
      ...
      storeContainer.getContainers.mockReturnValue([
        { ...container, labels: { 'dd.group': 'media' } },
      ]);
      if (kind === 'update') await trigger.flushDigestBuffer();
      else
        await trigger.handleSecurityScanCycleCompleteEvent({ cycleId: 'cycle', scannedCount: 1 });
      expect(send).not.toHaveBeenCalled();
      expect(notificationHistoryStore.recordNotification).not.toHaveBeenCalled();
      // Ineligible entries are evicted, not retained to send if membership later returns.
  ...
  test.each(['excluded', 'agent', 'threshold', 'rule', 'unknown'])(
    'does not admit a matching security digest when %s disallows delivery',
    async (reason) => {
      trigger.configuration.securitymode = 'digest';
      const container = groupedContainer();
      if (reason === 'excluded') container.notificationTriggerExclude = 'slack.grouped';
      if (reason === 'agent') trigger.agent = 'other';
      if (reason === 'threshold') trigger.configuration.threshold = 'minor-only';
      if (reason === 'rule') notificationStore.isTriggerEnabledForRule.mockReturnValue(false);
      const send = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
      await trigger.handleSecurityAlertEvent({
        containerName: container.name,
        container: reason === 'unknown' ? undefined : container,
        cycleId: 'cycle',
      });
      await trigger.handleSecurityScanCycleCompleteEvent({ cycleId: 'cycle', scannedCount: 1 });
      expect(send).not.toHaveBeenCalled();
  ...
  test.each(['update', 'security'])(
    'rechecks exclusions before %s digest delivery',
    async (kind) => {
      trigger.configuration.mode = 'digest';
      trigger.configuration.securitymode = 'digest';
      const send = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
      ...
      storeContainer.getContainers.mockReturnValue([
        { ...container, notificationTriggerExclude: 'slack.grouped' },
      ]);
      if (kind === 'update') await trigger.flushDigestBuffer();
      else
        await trigger.handleSecurityScanCycleCompleteEvent({ cycleId: 'cycle', scannedCount: 1 });
  ...
  test('keeps admitted security rows when the current store has an unrelated incomplete record', async () => {
    trigger.configuration.securitymode = 'digest';
    const send = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
    const container = groupedContainer();
    await trigger.handleSecurityAlertEvent({
    ...
    storeContainer.getContainers.mockReturnValue([{ name: 'incomplete' }]);
    await trigger.handleSecurityScanCycleCompleteEvent({ cycleId: 'cycle', scannedCount: 1 });
    expect(send).toHaveBeenCalledTimes(1);
    expect(send.mock.calls[0][0]).toHaveLength(1);
  });

  test.each(['same', 'changed', 'absent'])(
    'revalidates queued lifecycle batches with %s current membership',
    async (membership) => {
      vi.useFakeTimers();
      trigger.configuration.mode = 'batch';
      trigger.configuration.threshold = 'minor-only';
      const container = groupedContainer();
      const send = vi.spyOn(trigger, 'triggerBatch').mockResolvedValue(undefined);
      await trigger.handleContainerUpdateFailedEvent({
        containerName: container.name,
      ...
      storeContainer.getContainers.mockReturnValue(
        membership === 'absent'
          ...
          : [
              {
                ...container,
                labels: { 'dd.group': membership === 'changed' ? 'media' : 'payments' },
              },
            ],
      );
      await vi.advanceTimersByTimeAsync(250);
      expect(send).toHaveBeenCalledTimes(membership === 'changed' ? 0 : 1);
    },
  );
});
...
test('getMetadata should include trigger category for notification types', () => {
  trigger.type = 'slack';
  trigger.name = 'notify';
...
// cron scan passed the check in both and sent the same candidate twice.
test('handleContainerReports sends once for two overlapping batches of the same once=true history key', async () => {
  await trigger.register('trigger', 'test', 'trigger1', {
    ...configurationValid,
    mode: 'batch',
  });
  ...
  const batchSpy = vi.fn().mockReturnValue(sendPromise);
  trigger.triggerBatch = batchSpy;

  const call1 = trigger.handleContainerReports([{ changed: true, container }]);
  const call2 = trigger.handleContainerReports([{ changed: true, container }]);

  // call1 runs synchronously up to its triggerBatch() await, so call2 already
  // sees the reservation held when it evaluates the same report.
  ...
  // The release has to key on exactly what the reserve keyed on, or the
  // reservation outlives the batch and mutes this container for good.
  expect(trigger.inFlightOnceNotificationKeys.size).toBe(0);
});

test('handleContainerReports still batches a genuinely new result once the previous batch is done', async () => {
  await trigger.register('trigger', 'test', 'trigger1', {
    ...configurationValid,
    mode: 'batch',
  });
  ...
  await trigger.handleContainerReports([{ changed: true, container: updatedContainer }]);
  expect(batchSpy).toHaveBeenCalledTimes(2);
});

// Regression test for #972 on the digest path: the flush records history only
// after triggerBatch() resolves, so a report landing mid-flush read "not
// digested yet", replaced the buffer entry behind the send, and the identity
// check then skipped the post-send delete - so the next flush sent the exact
// same result again.
test('flushDigestBuffer sends once when a report for the same result lands mid-flush', async () => {
  ...
  const flush = trigger.flushDigestBuffer();
  expect(batchSpy).toHaveBeenCalledTimes(1);

  // A manual scan hands the same candidate to the same trigger while the
  // digest send is still in flight.
  await trigger.handleContainerReportDigest({ changed: true, container: { ...container } });

  resolveSend();
  await flush;

  // The mid-flush report did not replace the buffer entry, so the post-send
  // delete landed and the next flush has nothing left to send.
  await trigger.flushDigestBuffer();
  expect(batchSpy).toHaveBeenCalledTimes(1);
  ...
  // Both the buffer-time reservation and the flush's own have to be released
  // on keys that match what took them, or the container never buffers again.
  expect(trigger.inFlightOnceNotificationKeys.size).toBe(0);
});
...
// post-send delete then evicts the newer candidate the buffer was holding, so
// the real update is lost. The reservation now decides whether it goes out.
test('flushDigestBuffer skips and evicts an entry whose current store state was already digested', async () => {
  await trigger.register('trigger', 'test', 'digest-trigger', {
    ...configurationValid,
    mode: 'digest',
  });
  ...
  await trigger.handleContainerReportDigest({ changed: true, container: containerA });

  let resolveSend: () => void = () => undefined;
  const sendPromise = new Promise<void>((resolve) => {
    resolveSend = resolve;
  });
  const batchSpy = vi.fn().mockReturnValueOnce(sendPromise).mockResolvedValue(undefined);
  trigger.triggerBatch = batchSpy;

  // A digests, and a newer candidate B replaces the buffer entry mid-send, so
  // the identity-guarded post-send delete correctly leaves B buffered.
  const flushA = trigger.flushDigestBuffer();
  expect(batchSpy).toHaveBeenCalledTimes(1);
  await trigger.handleContainerReportDigest({ changed: true, container: containerB });
  resolveSend();
  await flushA;
  expect(trigger.digestBuffer.get('c1')).toBe(containerB);

  // A stale scan puts A back in the store, so the next flush substitutes the
  // already-digested A for the buffered B.
  storeContainer.getContainersRaw.mockReturnValue([containerA]);
  await trigger.flushDigestBuffer();
  ...
  // Nothing was lost: once the store catches up, B's own report re-buffers it
  // and the next flush sends it.
  storeContainer.getContainersRaw.mockReturnValue([containerB]);
  await trigger.handleContainerReportDigest({ changed: true, container: containerB });
  await trigger.flushDigestBuffer();
  expect(batchSpy).toHaveBeenCalledTimes(2);
});

test('flushDigestBuffer leaves a concurrently replaced buffer entry in place when the reservation fails', async () => {
  await trigger.register('trigger', 'test', 'digest-trigger', {
    ...configurationValid,
    mode: 'digest',
  });
  ...
  // Already digested, so the flush's reservation for the store container fails.
  notificationHistoryStore.recordNotification(
    trigger.getId(),
    'c1',
  ...
  // Replace the buffer entry after the flush snapshots it, the same way a
  // concurrent report would.
...
// Regression test for the batch retry buffer, which getBatchRetryContainers
// pulls into every batch until a send succeeds. The reservation on the report
// path does not cover it, so two overlapping batches after a failure both
// carried the same retry entry to triggerBatch.
test('handleContainerReports sends a retry-buffered container once across two overlapping batches', async () => {
  await trigger.register('trigger', 'test', 'trigger1', {
    ...configurationValid,
    mode: 'batch',
  });
  ...
  // A failed batch parks the container in the retry buffer.
  trigger.triggerBatch = vi.fn().mockRejectedValue(new Error('network timeout'));
  await trigger.handleContainerReports([{ changed: true, container }]);
  expect(trigger.batchRetryBuffer.size).toBe(1);

  // The store keeps the container alive so getBatchRetryContainers returns it
  // to both overlapping calls, which is what the reservation has to arbitrate.
  ...
  const call1 = trigger.handleContainerReports([]);
  const call2 = trigger.handleContainerReports([]);
  expect(batchSpy).toHaveBeenCalledTimes(1);
...
test('handleContainerReportDigest still buffers a genuinely new result after a flush', async () => {
  ...
  // The flush released its reservation, so the next candidate still buffers.
  const updatedContainer = {
    ...container,
    updateKind: { kind: 'tag', localValue: '2.0', remoteValue: '3.0' },
    result: { tag: '3.0' },
  };
  await trigger.handleContainerReportDigest({ changed: true, container: updatedContainer });
  await trigger.flushDigestBuffer();
  expect(batchSpy).toHaveBeenCalledTimes(2);
});
...
// #946 finding 3: the catch-up scan re-runs watch(), which is enough for the simple and
// batch paths because their dispatch hangs off the report it emits. A digest trigger's
// report handler only re-buffers, so without a flush the deferred install waited for the
// next digest cron - up to a day after the window that was supposed to apply it.
describe('maintenance window opening flushes a digest action buffer (#946)', () => {
  const bufferedContainer = {
    id: 'c1',
    name: 'app',
    watcher: 'local',
  ...
  function makeDigestActionTrigger() {
    const instance = new Trigger() as any;
    instance.log = trigger.log;
    instance.configuration = { ...configurationValid, mode: 'digest' };
    instance.type = 'docker';
    instance.name = 'update';
    return instance;
  }

  function bufferOne(instance: any, container = bufferedContainer) {
    instance.digestBuffer.set(container.id, container);
    instance.digestBufferUpdatedAt.set(container.id, Date.now());
    storeContainer.getContainersRaw.mockReturnValue([container]);
  }

  test('a deferred digest install lands when the window opens instead of at the next digest cron', async () => {
    ...
    bufferOne(actionTrigger);
    const { enqueueContainerUpdates: enqueueMock } = await import(
      '../../updates/request-update.js'
    );
    vi.mocked(enqueueMock).mockClear();
    vi.mocked(enqueueMock).mockResolvedValue({ accepted: [], rejected: [] } as any);

    // 08:00 digest cron, window closed: nothing is enqueued and the entry stays buffered.
    await actionTrigger.flushDigestBuffer();
    ...
    // 02:00 the window opens, the watcher runs its catch-up scan and announces it.
    windowOpen = true;
  ...
  test('records only the watchers that actually deferred in a mixed flush', async () => {
    const actionTrigger = makeDigestActionTrigger();
    const held = { ...bufferedContainer, id: 'c-held', name: 'held', watcher: 'closed' };
    const ready = { ...bufferedContainer, id: 'c-ready', name: 'ready', watcher: 'open' };
    mockRegistryGetState.mockReturnValue({
      watcher: {
        'docker.closed': {
          type: 'docker',
    ...
    for (const container of [held, ready]) {
      actionTrigger.digestBuffer.set(container.id, container);
      actionTrigger.digestBufferUpdatedAt.set(container.id, Date.now());
    }
    storeContainer.getContainersRaw.mockReturnValue([held, ready]);
    const { enqueueContainerUpdates: enqueueMock } = await import(
      '../../updates/request-update.js'
    );
    vi.mocked(enqueueMock).mockResolvedValue({ accepted: [], rejected: [] } as any);
    ...
    expect([...actionTrigger.maintenanceWindowDeferredDigestWatchers]).toEqual(['docker.closed']);
    expect(actionTrigger.digestBuffer.size).toBe(1);
  });

  test('a window opening on an unrelated watcher does not bring the digest forward', async () => {
    const actionTrigger = makeDigestActionTrigger();
    mockRegistryGetState.mockReturnValue({
      watcher: {
        'docker.local': {
    ...
    await actionTrigger.handleMaintenanceWindowOpenedEvent({ watcherId: 'docker.elsewhere' });

    expect(flushSpy).not.toHaveBeenCalled();
    expect(actionTrigger.digestBuffer.size).toBe(1);
  });

  test('a flush with nothing deferred arms no window opening at all', async () => {
    const actionTrigger = makeDigestActionTrigger();
    mockRegistryGetState.mockReturnValue({
      watcher: {},
      trigger: {},
  ...
  test('a deferred container whose watcher cannot be identified arms no window opening', async () => {
    const actionTrigger = makeDigestActionTrigger();
    const idLessWatcher = { ...bufferedContainer, watcher: '' };
    bufferOne(actionTrigger, idLessWatcher);
    actionTrigger.runAcceptedUpdateBatch = vi
      .fn()
      .mockResolvedValue({ dispatched: true, deferredIds: new Set(['c1']) });
  ...
  test('a digest trigger registers for window openings and drops the handler on deregister', async () => {
    ...
    vi.mocked(event.registerContainerReport).mockReturnValue(vi.fn());
    vi.mocked(event.registerSecurityScanCycleComplete).mockReturnValue(vi.fn());
    vi.mocked(event.registerMaintenanceWindowOpened).mockReturnValue(unregister);

    const actionTrigger = makeDigestActionTrigger();
    await actionTrigger.register('trigger', 'docker', 'update', {
      ...configurationValid,
      mode: 'digest',
    });

    expect(event.registerMaintenanceWindowOpened).toHaveBeenCalled();
    const handler = vi.mocked(event.registerMaintenanceWindowOpened).mock.calls[0][0];
    const handlerSpy = vi
      .spyOn(actionTrigger, 'handleMaintenanceWindowOpenedEvent')
      .mockResolvedValue(undefined);
    await handler({ watcherId: 'docker.local' });
    expect(handlerSpy).toHaveBeenCalledWith({ watcherId: 'docker.local' });

    actionTrigger.maintenanceWindowDeferredDigestWatchers.add('docker.local');
    await actionTrigger.deregisterComponent();

    expect(unregister).toHaveBeenCalled();
    expect(actionTrigger.maintenanceWindowDeferredDigestWatchers.size).toBe(0);
  });

  test('a simple-mode trigger never registers for window openings', async () => {
    vi.mocked(event.registerContainerReport).mockReturnValue(vi.fn());
    vi.mocked(event.registerSecurityScanCycleComplete).mockReturnValue(vi.fn());
    vi.mocked(event.registerMaintenanceWindowOpened).mockReturnValue(vi.fn());

    const simpleTrigger = new Trigger() as any;
    simpleTrigger.log = trigger.log;
    await simpleTrigger.register('trigger', 'docker', 'simple-update', {
      ...configurationValid,
      mode: 'simple',
    });

    expect(event.registerMaintenanceWindowOpened).not.toHaveBeenCalled();
  });
});
