...
| `DD_TRIGGER_DOCKERCOMPOSE_{trigger_name}_ENGINE` | ⚪ | Runtime update engine | `dockerapi`, `composecli` | `dockerapi` |
...
| `DD_TRIGGER_DOCKERCOMPOSE_{trigger_name}_COMPOSETIMEOUT` | ⚪ | Timeout (ms) for each Compose CLI command when `ENGINE=composecli` | Integer >= `1000` | `300000` |
...
### Option 3: Run as root (quick fix)

Set `DD_RUN_AS_ROOT=true` to skip the privilege drop:
...
<Callout type="warn">Running as root trades the privilege-drop security boundary for full file access. This is fine for home-lab setups but may not meet your security requirements.</Callout>
