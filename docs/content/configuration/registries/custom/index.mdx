...
| `DD_REGISTRY_CUSTOM_{REGISTRY_NAME}_CAFILE` | ⚪ | Path to a custom CA bundle (PEM) for private/self-signed certificates | Absolute or relative file path in the container | |
| `DD_REGISTRY_CUSTOM_{REGISTRY_NAME}_INSECURE` | ⚪ | Disable TLS certificate verification (use only for trusted internal networks) | `true`, `false` | `false` |
