...
<Callout type="info">Migration behavior: `hub.public` keeps backward-compatible startup for legacy partial/mixed token-auth inputs (`LOGIN` only, `TOKEN` only, `PASSWORD` only, or `LOGIN` + `PASSWORD` + `TOKEN`) by falling back to anonymous access with a warning. This compatibility fallback is deprecated and will be removed in a future major release.</Callout>
