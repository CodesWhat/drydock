...
- **当存储卷拒绝 `chmod` 时启动不再崩溃** — 1.6.0 版本引入的权限加固在遇到 `EPERM` 时会抛出异常，导致拒绝 `chmod` 的挂载（NFS/CIFS 卷、非 root 容器）在启动时使整个进程崩溃，并彻底阻塞从 1.6.0 的升级；现在遇到 `EPERM`/`EACCES`/`ENOTSUP`/`EROFS` 时只会发出警告并继续运行。([#874](https://github.com/CodesWhat/drydock/discussions/874)、[#886](https://github.com/CodesWhat/drydock/pull/886))
