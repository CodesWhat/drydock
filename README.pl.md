...
**Obserwator aktualizacji obrazów kontenerów, 23 rejestry, 21 dostawców powiadomień i działań.**
...
Większość narzędzi wymusza kompromis. Automatyczne aktualizacje (Watchtower, Ouroboros) pobierają się i uruchamiają ponownie przy niewielkiej widoczności lub kontroli i obecnie w dużej mierze nie są konserwowane. Pulpity nawigacyjne (Portainer) zarządzają kontenerami, ale nie są zbudowane pod kątem analizy aktualizacji. Drydock to **najpierw monitor**: obserwuje 23 rejestry i dokładnie informuje Cię, co się zmieniło (główne, poboczne, poprawki lub podsumowanie), zanim cokolwiek się wydarzy, a następnie działa tylko wtedy, gdy na to pozwolisz. I sięga dalej niż którykolwiek z nich. Skanowanie pod kątem luk w zabezpieczeniach Trivy/Grype blokuje niebezpieczne aktualizacje, cosign weryfikuje podpisy, kopie zapasowe obrazów przed aktualizacją przywracają się automatycznie w przypadku niepowodzenia kontroli stanu, rozproszoni agenci obsługują zdalne hosty, a 21 integracji powiadomień i działań zamyka pętlę. Pełny cykl życia aktualizacji z interfejsem internetowym i interfejsem API REST.
...
| 🔔  | **21 wyzwalaczy**                                     | 17 kanałów powiadomień (Slack, Discord, Telegram, Teams, SMTP, MQTT, ntfy i więcej) oraz Docker, Docker Compose, Portainer i akcje poleceń, z szablonami dla poszczególnych zdarzeń/dostawców, podglądem na żywo, filtrowaniem progów i trybem wsadowym.                                                                                                                                                                                                                                                                                               |
...
Doker · Docker Compose · Portainer · Command
...
<tr><td>Kanały wyzwalaczy / powiadomień</td><td align="center">21</td><td align="center">17</td><td align="center">17</td><td align="center">~20</td></tr>
...
<tr><td>Uwierzytelnianie OIDC / SSO</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td><td align="center">✅</td></tr>
<tr><td>Kanały wyzwalaczy / powiadomień</td><td align="center">21</td><td align="center">11+</td><td align="center">5</td><td align="center">15+</td></tr>
...
| **v1.7.0** | Inteligentne aktualizacje i UX | Kolejność aktualizacji uwzględniająca zależności ([#219](https://github.com/CodesWhat/drydock/discussions/219)), zasady aktualizacji według akcji ([#511](https://github.com/CodesWhat/drydock/discussions/511)), ujednolicony zegar dojrzałości/wieku aktualizacji, klikalne łącza do portów, skróty klawiaturowe, PWA, poprawa kontrastu ciemnego motywu (WCAG 2.2) ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865)), usuwanie `DD_TRIGGER_*` (koniec okresu wycofywania dla wersji 1.5.0), curl usunięty z obrazu |
| **v1.8.0** | Zarządzanie flotą i konfiguracja na żywo | **Włączone do `dev/v1.8`, nieopublikowane:** SQLite, kolejka zatwierdzania, konfiguracja YAML, ograniczone edytory polityk obserwatorów/powiadomień/akcji i szablony podsumowań bezpieczeństwa, równoległe i selektywne aktualizacje zbiorcze ([#232](https://github.com/CodesWhat/drydock/discussions/232)), interfejs zależności, inwentaryzacja i czyszczenie obrazów, stan floty i odświeżanie inwentarza, udostępnianie grupowania etykietami, okna serwisowe tylko dla instalacji ([#946](https://github.com/CodesWhat/drydock/discussions/946)), własne uwierzytelnianie, klucze API z ograniczonym zakresem ([#469](https://github.com/CodesWhat/drydock/discussions/469)) oraz postęp/urządzenia Home Assistant ([#210](https://github.com/CodesWhat/drydock/discussions/210)). **W przeglądzie:** routing według dokładnej grupy tylko dla powiadomień ([#1251](https://github.com/CodesWhat/drydock/pull/1251)). **Pozostało:** Szersza konfiguracja na żywo i dziedziczone polityki grup oraz akcji wymagają decyzji o zarządzaniu ustawieniami; TOTP wymaga zatwierdzonego kontraktu; obserwator statycznych list obrazów czeka do wydania GA. |
| **v2.0+** | Rozszerzanie platformy i nie tylko | Obserwatorzy roju/Kubernetes, synchronizacja GitOps, przeglądarka woluminów, lokalnie zbudowane obrazy monitorowane względem zadeklarowanej bazy upstream ([#897](https://github.com/CodesWhat/drydock/discussions/897)), bramki kondycji, wdrożenia Canary, terminal webowy, RBAC, LDAP/AD, podstawowe wsparcie dla Podman za pośrednictwem jego interfejsu API zgodnego z Dockerem, CLI, wzmocniony obraz Wolfi, proxy gniazda |

**Otwarte wymagania społeczności:** Aktualizacje i restarty zależności między hostami ([#219](https://github.com/CodesWhat/drydock/discussions/219)), dojrzałość oparta na liczbie wydań ([#558](https://github.com/CodesWhat/drydock/discussions/558)) oraz domyślne ustawienie sprawdzania digestów dla całej floty ([#657](https://github.com/CodesWhat/drydock/discussions/657)) nie są zaimplementowane. Kolejność zależności na jednym hoście i jej nowy interfejs nie realizują prośby dotyczącej zdalnych hostów. Wykrywanie lokalnych kompilacji i bezpieczny wybór akcji ponownego budowania pozostają przyszłymi pracami ([#897](https://github.com/CodesWhat/drydock/discussions/897)); żaden z tych wpisów nie jest zobowiązaniem do konkretnej daty wydania.
