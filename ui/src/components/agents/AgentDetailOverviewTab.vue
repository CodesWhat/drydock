...
<template>
  <div class="p-4 space-y-5">
    ...
    <div>
      ...
      <div class="space-y-2">
        <div class="px-2.5 py-2 dd-rounded text-[11px]" :style="{ backgroundColor: 'var(--dd-bg-inset)' }">
          <div class="flex items-center justify-between gap-2">
            <span class="font-semibold dd-text">Watchers</span>
            <span class="text-[10px] dd-text-muted">{{ props.agent.watchers.length }}</span>
          </div>
          <div class="mt-1.5 flex flex-wrap gap-1.5">
            ...
            <span v-if="props.agent.watchers.length === 0" class="text-[10px] italic dd-text-muted">
              None
            </span>
          </div>
        </div>
        ...
        <div class="px-2.5 py-2 dd-rounded text-[11px]" :style="{ backgroundColor: 'var(--dd-bg-inset)' }">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-[10px] dd-text-muted">{{ props.agent.triggers.length }}</span>
          </div>
          <div class="mt-1.5 flex flex-wrap gap-1.5">
            <span
            ...
            <span v-if="props.agent.triggers.length === 0" class="text-[10px] italic dd-text-muted">
              None
            </span>
          </div>
        </div>
