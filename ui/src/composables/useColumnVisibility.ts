...
const visibleColumns = ref<Set<string>>(
  new Set(JSON.parse(localStorage.getItem('dd-table-cols') || 'null') || defaultVisibleCols),
);
watch(visibleColumns, (v) => localStorage.setItem('dd-table-cols', JSON.stringify([...v])), {
  deep: true,
});
