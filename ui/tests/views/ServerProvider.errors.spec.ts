...
// Isolate the persistent stream transport; all HTTP services and consumers remain real.
...
function successfulResponse(input: RequestInfo | URL): Response {
  ...
  if (input === '/api/v1/containers/recent-status')
    return Response.json({ statuses: {}, statusesByIdentity: {} });
...
describe.each(['fr', 'ar'] as const)('server/provider failures in %s', (locale) => {
  beforeEach(() => {
    i18n.global.locale.value = locale;
  });

  it.each([
    { name: 'server', url: '/api/v1/server', key: 'serversView.loadError' },
    { name: 'watchers', url: '/api/v1/watchers', key: 'watchersView.loadError' },
    { name: 'registries', url: '/api/v1/registries', key: 'registriesView.loadError' },
  ])(
    'renders the actual $name error on Dashboard and recovers with its Retry button',
    async ({ url, key }) => {
      let failing = true;
  ...
  it.each([
    { url: '/api/v1/server', key: 'serversView.loadError' },
    { url: '/api/v1/watchers', key: 'watchersView.loadError' },
  ])('renders $url failure on Servers', async ({ url, key }) => {
    vi.mocked(fetch).mockImplementation(async (input) =>
      input === url
        ? new Response(null, { status: 502, statusText: 'Upstream unavailable' })
        : successfulResponse(input),
    );
    const view = await mountView(ServersView);
    expect(view.text()).toContain(`${i18n.global.t(key)} (HTTP 502): Upstream unavailable`);
    expect(vi.mocked(fetch).mock.calls.filter(([input]) => input === url)).toHaveLength(1);
  });

  it('retains rendered Dashboard data after background failures and keeps the static TTL', async () => {
    let failServer = false;
    vi.mocked(fetch).mockImplementation(async (input) =>
      input === '/api/v1/server' && failServer
    ...
    expect(
      vi.mocked(fetch).mock.calls.filter(([input]) => input === '/api/v1/server'),
    ).toHaveLength(1);
    await data.fetchDashboardData({ background: true });
    expect(data.error.value).toBeNull();
    expect(data.serverInfo.value).toEqual(server);
    expect(debug).toHaveBeenCalledWith(`${i18n.global.t('serversView.loadError')} (HTTP 503)`);
  });
});
