        ...
        <div className="rounded-xl border border-neutral-200 bg-white/70 p-8 shadow-sm backdrop-blur-sm sm:p-12 dark:border-neutral-800 dark:bg-neutral-950/70">
          ...
          <div className="mb-12 rounded-lg border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-800/50 dark:bg-emerald-950/30">
            ...
            <p className="text-emerald-800 dark:text-emerald-300">
              We have audited our entire CI/CD pipeline, dependency tree, and runtime integrations.
              Drydock does not use the compromised GitHub Actions, did not pull the backdoored
              binary, and all workflow actions are pinned by immutable commit SHA. No user action is
              required for Drydock itself.
            </p>
          </div>
          ...
          <div className="prose prose-neutral max-w-none dark:prose-invert prose-headings:font-semibold prose-a:text-blue-600 dark:prose-a:text-blue-400">
            ...
            <ul>
              ...
              <li>
                <strong>Trivy binary v0.69.4</strong> &mdash; a backdoored release was published via
            ...
            <p>
              The root cause was a continuation of an earlier supply chain attack from late February
              2026. Aqua Security rotated credentials but the process was not atomic &mdash; the
              attacker was able to exfiltrate newly rotated secrets during the rotation window,
              retaining write access to the repositories.
            </p>
            <p>
...
git push -f origin refs/tags/0.24.0`}</code>
            ...
            <p>
              Since GitHub&apos;s release page doesn&apos;t visually change when a tag is repointed,
              any workflow referencing these actions by version tag (e.g., <code>@0.24.0</code>)
              would silently execute the malicious payload on its next run.
            </p>
            ...
            <ol>
              ...
              <li>
                ...
                paths for SSH keys, cloud credentials (AWS, GCP, Azure), Kubernetes tokens, database
                configs, <code>.env</code> files, TLS private keys, and cryptocurrency wallets
              </li>
              <li>
                <strong>Encrypted exfiltration</strong> &mdash; data was AES-256-CBC encrypted with
                an RSA-4096 wrapped session key and POSTed to a typosquatted domain (
              ...
              <li>
                <strong>Cleanup</strong> &mdash; all temporary files were removed and the legitimate
                Trivy scanner ran normally, making the compromise invisible in workflow logs
              </li>
            </ol>
            ...
            <p>
              Drydock&apos;s Docker image includes Trivy and cosign binaries for local vulnerability
              scanning and image signature verification. The Trivy binary is installed from
              Alpine&apos;s package repository and pinned at <strong>v0.69.3-r1</strong> in the
              Dockerfile, which predates the compromised v0.69.4 release. Aqua Security has
              confirmed that v0.69.3 is safe and protected by GitHub&apos;s immutable releases
              feature.
            </p>
            <pre>
...
apk add --no-cache --repository=https://dl-cdn.alpinelinux.org/alpine/edge/testing trivy=0.69.3-r1`}</code>
            ...
            <p>
              <strong>Correction (March 22):</strong> Our initial audit stated the Dockerfile pinned
              Trivy, but the <code>main</code> branch Dockerfile did not include an explicit version
              constraint for the Trivy APK package at the time of publication. This does not change
              the outcome: no Docker image builds ran during the exposure window, the Alpine
              repository was serving v0.69.3 (safe), and even if a compromised Trivy binary had been
              installed, it would only be used to scan container images for CVEs &mdash; it has no
              access to CI runner secrets, SSH keys, process memory, or any of the targets the
              GitHub Actions payload exploited. The QA test compose file also referenced{" "}
              <code>aquasec/trivy:latest</code>, but that environment only runs on developer
              machines, never in CI or production. As a defense-in-depth measure, we have pinned
              both: <code>trivy=0.69.3-r1</code> in the Dockerfile and{" "}
...
uses: docker/build-push-action@d08e5c354a6adb9ed34480a06d141179aa583294    # v7.0.0`}</code>
            ...
            <ul>
              ...
              <li>
                <strong>OpenSSF Scorecard</strong> &mdash; automated security health checks on every
                push
              </li>
              <li>
                <strong>Dependency review</strong> &mdash;{" "}
                <code>actions/dependency-review-action</code> audits dependency changes on every PR
              </li>
              <li>
                <strong>Cosign image signing</strong> &mdash; all published Docker images are signed
                with Sigstore cosign and include build provenance attestations
              </li>
            </ul>

            <h2>Verifying your Drydock installation</h2>
            <p>
              Drydock bundles Trivy and cosign in its Docker image for local vulnerability scanning
              and image signature verification. All official Drydock releases pin Trivy at
              v0.69.3-r1, which is confirmed safe. You can verify your running instance:
            </p>
            <pre>
...
# Compromised version: v0.69.4`}</code>
            ...
            <p>
              If you use an external Trivy server via <code>DD_SECURITY_TRIVY_SERVER</code>, verify
              that server independently as well.
            </p>
            ...
            <p>
              ...
              all secrets accessible to those runners as compromised. Rotate credentials immediately
              and follow{" "}
              <a
            ...
            <p>
              Verify your Trivy version is not v0.69.4. Update to v0.69.3 or wait for the next clean
              release. Check for the persistence mechanism: look for{" "}
            ...
            <p>
              This is the single most effective defense against tag repointing attacks. Git tags are
              mutable pointers &mdash; they can be silently redirected to malicious commits without
              any visible change on the release page. SHA pins are immutable.
            </p>
            <pre>
            ...
            <ul>
              ...
              <li>
                <a
                  ...
                  rel="noopener noreferrer"
                >
                  CrowdStrike &mdash; From Scanner to Stealer: Inside the trivy-action Supply Chain
                  Compromise
                </a>
              </li>
              <li>
              ...
              <table className="w-auto">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>Mar 19, 17:43</td>
                    <td>
                      Attacker force-pushes 76 trivy-action tags and 7 setup-trivy tags to malicious
                      commits
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td>Mar 22</td>
                    <td>Drydock completes audit, publishes this advisory and community response</td>
                  </tr>
                  <tr>
                    <td>Mar 22 (update)</td>
                    <td>
