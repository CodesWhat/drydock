...
- **v1.5.0 cleanup backlog** (open code-quality nits — fold into the v1.6.0 perf+hardening pass):
    ...
    - ~~Render-path allocation in `ContainersGroupedViews.vue` + `DataCardGrid.vue:31` / `DataListAccordion.vue:30` v-memo array fingerprint~~ — **superseded:** v1.6 "table-only UI simplification" will delete `DataCardGrid` and `DataListAccordion` outright.
- **★ v1.6.0 — User-Requested Features (pull-forward, ship first):** consolidated from a 2026-06-01 fan-out triage of all open Discussions (full verdicts in `current-tracker.md`). Ordered by demand × visible-win ÷ effort. Goal: land a visible block of community-requested wins in the next release. Items 1–4 already lived in v1.6 lanes below — promoted here so they ship as a coherent set, not buried. Items 5–6 are pulled forward from later versions; items 7–8 are documented here but **stay** in v1.7 (effort/dependency reasons noted).
...
- `v1.6.0`: **Notifications, Policy & Release Intel — implemented and verified locally on `dev/v1.6`:** zero-dependency custom dashboard grid (#281), per-rule/per-trigger notification templates + preview, audit-backed notification-bell preferences, cross-device preference sync, software-version visibility, bidirectional MQTT for HA, declarative update-policy precedence and revert UX (#320 / #307), maturity stabilization countdown, pinned-tag policy inheritance (#498), health-status notifications (#198), trigger-taxonomy Phase 3 error-level warnings, responsive table/card list views, compose-file matching robustness (#365), Bucket C hardening, and v1.6 compatibility removals. **Still open before GA or explicit re-laning:** #490 long-scan correctness, #325 Update Status panel/update mode, #498's stacked at-a-glance tag insight UI, scanner runtime decoupling/Grype, SBOM off-heap storage, and the remaining #321 UX/storage slices. Do not call those shipped.
...
**Audit status (2026-07-12):** the current runtime is still Trivy-only and SBOM documents remain inline. Scanner backend decoupling, Grype, asset lifecycle, and the #490 long-scan fixes below must not appear in public completed-v1.6 scope unless they land and pass verification.
...
### 4.5.3.5 Scanner Long-Scan UX — 🔴 #490 open, required before v1.6 GA or explicit re-lane

**Timeline target:** v1.6.0 (queued post v1.5-rc17). Driven by QA findings: large images (multi-GB ML/CUDA) hit the default 120s Trivy timeout, and the lifecycle blocks silently with no progress indication. Cache-reuse in the gate path was wired in v1.5-rc17; the items here cover what remains. **Now also the fix package for [#490](https://github.com/CodesWhat/drydock/issues/490)** (user report 2026-07-06, v1.5.1-rc.5: intermittent "Trivy command failed (exit=unknown)" fails updates on large images; retry succeeds). Diagnosis workflow `wf_947d6e33` verified the mechanics — see the four #490 bullets added below. Reporter told fix lands in v1.6; GA release notes carry a known-issue + `DD_SECURITY_TRIVY_TIMEOUT` workaround note. DO NOT drop these when trimming scope.
...
- **Default Trivy timeout bump.** Raise `DD_SECURITY_TRIVY_TIMEOUT` default from 120s to 600s. Document the override in `content/docs/current/configuration/security`. Bumping the default avoids QA reports of "scan failed at 2 min" on legitimately large images while leaving the env var available to tighten for fast-path images.
...
- **(#490) Fix the double-timeout-layer race.** `scan.ts` passes the same `configuration.trivy.timeout` value to BOTH trivy's `--timeout` flag (scan.ts:287-288) and Node's `execFile` timeout option (scan.ts:237). Node's clock starts before the trivy binary even boots, so Node always wins the race, SIGTERMs the child, and produces the misleading `exit=unknown` (scan.ts:242 — signal kill leaves `error.code` null and `child.exitCode` null). Give the Node-side timeout deliberate headroom (e.g. trivy timeout + 30s) so trivy's own timeout fires first and reports a real exit code + clear stderr. Genuine correctness bug independent of the default-value debate.
- **(#490) Don't prune the pulled image on scan *error*.** `SecurityGate.ts:592-604` prunes the just-pulled image on any gate failure — `prune.onBlock` semantics firing on transient scan errors too, so the user's retry pays a full re-pull. Scope pruning to genuine `security-scan-blocked` outcomes (or add `DD_SECURITY_PRUNE_ONERROR`, default false).
- **(#490) Retry once on transient scan error.** In the gate path, a scan result of status `error` (NOT `blocked`) gets one automatic retry before failing the update — empirically the second attempt succeeds once trivy's DB/layer cache is warm. Keeps fail-closed semantics for real vulnerability blocks.
- **(#490) Cross-link `--server` mode from the timeout docs.** `DD_SECURITY_TRIVY_SERVER` already works (scan.ts:295-297, documented at configuration/security example); the timeout section should point large-image/slow-host users at it. Docs-only.
- **(#490) DB warmup** is covered by 4.5.3.4's "Warm scanner databases on demand" bullet — the first scan after container start currently pays the trivy vulnerability-DB download inside the same fixed scan budget; decouple it (startup/periodic `--download-db-only` style warmup).
