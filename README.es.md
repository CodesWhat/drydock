...
Esta versión también actualiza las dependencias y el paquete de zonas horarias de Alpine, restaura el registro de referencias del sitio web respetando la privacidad y corrige la selección de la rama de Crowdin y las comprobaciones de dependencias. Los cambios en tiempo de ejecución requieren un nuevo periodo de pruebas de siete días. Los mensajes MQTT retenidos antiguos no se eliminan automáticamente; comprueba si hay otros publicadores antes de borrar estados obsoletos.
...
- **Un controlador con su propio watcher `local` ya no rechaza todos los contenedores que un agente reporta bajo un watcher con el mismo nombre.** La propiedad de un contenedor sin fila en el almacén ahora se decide por lo que los propios watchers del controlador han enumerado realmente, no por una coincidencia de nombre. ([#1018](https://github.com/CodesWhat/drydock/pull/1018))
- **Una reversión de un contenedor gestionado por Compose ya no vuelve a desplegar la actualización que se suponía debía deshacer.** La recreación de Compose ahora pasa la imagen recibida al refresco en tiempo de ejecución en lugar de derivar una del propio candidato de actualización del contenedor; la reversión automática entrega al monitor de salud el contenedor completo en lugar de solo su id y nombre, y la imagen de respaldo se descarga, fijada por digest cuando el registro lo indica, antes de detener o eliminar nada. ([#1023](https://github.com/CodesWhat/drydock/pull/1023), [#1029](https://github.com/CodesWhat/drydock/pull/1029))
- **La imagen arm64 publicada vuelve a ser una imagen arm64 real**, en lugar de una imagen x86-64 con etiqueta arm64. Ambos anclajes de imagen base vuelven a apuntar a digests del índice de imagen multiarquitectura, y la versión ahora rechaza firmar, etiquetar o promover una imagen cuyos binarios no coincidan con la plataforma en la que se publica. ([#1024](https://github.com/CodesWhat/drydock/pull/1024))
- **Un contenedor movido a un agente ya no queda varado cuando se apaga el watcher local del controlador.** La poda de inicio que limpia los registros obsoletos propiedad del controlador ahora también se ejecuta cuando nunca se configuró un watcher local, de modo que el reporte del agente ya no se rechaza como propiedad del controlador. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Un contenedor entregado del controlador a un agente ya no llega con su snooze, modo de madurez y etiquetas omitidas restablecidos a los valores predeterminados.** La poda de inicio ahora guarda la política de actualización bajo el id de Docker del contenedor, que el traslado deja intacto, en lugar de una clave que el registro entrante nunca podría haber consultado. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **Un watcher local que falla al registrarse ya no borra los registros de sus contenedores cuando un segundo watcher sí se registra.** La poda ahora conserva cualquier registro cuyo watcher siga configurado, se haya registrado o no en esta ejecución, y espera a que cada registro termine antes de leer el registro. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **La página de agentes ahora dice que los registries deben configurarse en cada agente, no solo en el controlador**, con un ejemplo completo de Gitea en ambos lados. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))
- **El ejemplo de registries de la página de agentes ahora monta el archivo CA al que hace referencia**, en lugar de apuntar `DD_AGENT_REMOTE1_CAFILE` a una ruta que el servicio de Compose nunca montó. ([#1037](https://github.com/CodesWhat/drydock/pull/1037))
- **La página de watchers ahora dice que la ventana de mantenimiento bloquea todo el escaneo programado, no solo la instalación de una actualización.** Una ventana cerrada deja los contenedores nuevos invisibles, el estado del contenedor desactualizado y las notificaciones de actualización aplazadas hasta que vuelve a abrirse; un escaneo manual sigue omitiéndola igual que una actualización manual. ([#1010](https://github.com/CodesWhat/drydock/pull/1010))

Notas completas de la versión en [CHANGELOG.md](./CHANGELOG.md#170-rc11--2026-09-05).
...
| **v1.7.0** | Actualizaciones inteligentes y UX | Ordenamiento de actualizaciones consciente de la dependencia ([#219](https://github.com/CodesWhat/drydock/discussions/219)), política de actualización por acción ([#511](https://github.com/CodesWhat/drydock/discussions/511)), reloj unificado de madurez/antigüedad de actualizaciones, enlaces de puertos en los que se puede hacer clic, atajos de teclado, PWA, revisión de contraste del tema oscuro (WCAG 2.2) ([#850](https://github.com/CodesWhat/drydock/issues/850), [#865](https://github.com/CodesWhat/drydock/discussions/865)), eliminación de `DD_TRIGGER_*` (fin de la ventana de obsolescencia de v1.5.0), curl eliminado de la imagen |
| **v1.8.0** | Gestión de flotas y configuración en vivo | Migración de la tienda SQLite, cola de aprobación por actualización, configuración YAML, configuración de UI en vivo, actualizaciones paralelas, actualizaciones masivas selectivas ([#232](https://github.com/CodesWhat/drydock/discussions/232)), UI de ordenamiento de dependencias, administrador y eliminación de imágenes, observador de listas de imágenes estáticas, ventana de mantenimiento limitada a instalaciones de forma predeterminada ([#946](https://github.com/CodesWhat/drydock/discussions/946)), claves API giratorias con alcance (tokens de portador estáticos para integraciones de HA/panel, [#469](https://github.com/CodesWhat/drydock/discussions/469)), inicio de sesión de dos factores TOTP, progreso de actualización de Home Assistant + dispositivos por contenedor ([#210](https://github.com/CodesWhat/drydock/discussions/210)) |
| **v2.0+** | Expansión de plataforma y más allá | Vigilantes de enjambre/Kubernetes, sincronización GitOps, navegador de volúmenes, imágenes creadas localmente monitoreadas contra una base upstream declarada ([#897](https://github.com/CodesWhat/drydock/discussions/897)), puertas de estado, implementaciones canary, terminal web, RBAC, LDAP/AD, compatibilidad básica con Podman a través de su API compatible con Docker, CLI, imagen reforzada de Wolfi, proxy de socket |
