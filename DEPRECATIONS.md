...
**Migration:** Read and write store state through the API (`GET/PATCH /api/v1/settings`, `/api/v1/containers`, and so on) rather than `dd.json` directly. A backup or restore tool that copied `dd.json` off disk should copy `dd.sqlite` (and its `-wal`/`-shm` sidecars, if present) instead; `GET /api/v1/store` reports the current file names under `configuration.file` (legacy, still resolved for the one-time import) and `configuration.dbFile`.
