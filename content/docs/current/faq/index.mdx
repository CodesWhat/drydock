...
**Recommended fix:** Use a [Docker socket proxy](/docs/configuration/watchers#option-2-socket-proxy-recommended) so drydock never needs direct socket access.
...
Drydock supports two log formats controlled by environment variables:
...
Set `DD_LOG_FORMAT=json` for structured output suitable for log aggregators like Elasticsearch or Loki. See [Logs](/docs/configuration/logs) for examples.
