...
If updates still do not appear, verify both minimum versions and inspect the watcher descriptor for all three exact marker values. Older Portwing agents, partial/different marker values, and Docker Compose actions stay on the legacy remote-trigger path; Portwing intentionally advertises no remote trigger and its legacy watcher/trigger POST endpoints return 501. See [Edge-agent dial-out (Portwing)](/docs/configuration/agents#edge-agent-dial-out-portwing) and the [Portwing transport reference](/docs/api/portwing#controller-owned-docker-watcher-and-updates).
