...
In versions prior to 1.5.3, a failure that happened *after* the replacement container was created (but before it was confirmed healthy) could leave that orphaned "Created" container squatting the original name, with the old container still parked under the `-old-<timestamp>` suffix. A subsequent update attempt against the renamed container would then fail cleanly rather than compounding the problem.

As of 1.5.3, Drydock detects and removes the orphaned replacement container as part of rollback, so the rename back to the original name succeeds automatically. If you're on an older version and hit this:
...
Upgrading to 1.5.3 or later prevents this from happening again. Note that if you intentionally gave a container a name matching this pattern (`<name>-old-<10+ digit number>`), Drydock can't tell it apart from its own rollback rename and will refuse to update it — rename the container to something outside that pattern to make it updatable again.
