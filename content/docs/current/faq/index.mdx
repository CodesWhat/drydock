...
Set `DD_LOG_LEVEL=debug` and look for one of the skip decisions a trigger logs before dropping an event -- threshold not reached, `dd.notification.include`/`dd.notification.exclude` label mismatch, `ONCE=true` dedup, a disabled notification rule, or a failed provider call queued in the outbox. Each skip line carries a `component` field naming the trigger (`trigger.<type>.<name>`) and, for most of them, a `container` field naming the container. See [Diagnosing a missed notification](/docs/configuration/triggers#diagnosing-a-missed-notification) for the full table of log lines to grep for and a worked example.
