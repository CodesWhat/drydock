...
In versions prior to 1.5.2, a failure that happened *after* the replacement container was created (but before it was confirmed healthy) could leave that orphaned "Created" container squatting the original name, with the old container still parked under the `-old-<timestamp>` suffix. A subsequent update attempt against the renamed container would then fail cleanly rather than compounding the problem.

As of 1.5.2, Drydock detects and removes the orphaned replacement container as part of rollback, so the rename back to the original name succeeds automatically. If you're on an older version and hit this:

1. Confirm which container currently holds the original name: `docker ps -a --filter name=<name>`
2. If a `Created`-state container is squatting the name, remove it: `docker rm <container-id>`
3. Rename the `-old-<timestamp>` container back: `docker rename <name>-old-<timestamp> <name>`
