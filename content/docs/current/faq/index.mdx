...
This applies to floating tags (`latest`, `v3`, `16-alpine`) as well as fully-pinned specific tags (e.g. `1.4.5`, `v1.13.3`) — a pinned container compares by digest by default so a rebuilt image republished under the same tag is still detected, but it will not climb to a different tag on its own. Since v1.5.2, this doesn't leave you blind to what's out there: drydock still computes an informational `updateInsight` for pinned containers — the best newer same-family tag available in the registry, surfaced in the UI as a neutral "Pinned" state with a stacked current → newer tag view and a Major/Minor/Patch pill (hover for details). It's purely informational; it never triggers an update, a notification, or changes the actionable update state. Disable it per watcher with `DD_WATCHER_{watcher_name}_TAG_PIN_INFO=false`.
