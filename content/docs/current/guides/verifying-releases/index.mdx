...
### Verify the container SBOM attestation

The container image also carries a signed Software Bill of Materials, generated with [Trivy](https://trivy.dev/) in [SPDX 2.3](https://spdx.dev/) format and attested the same way as the provenance above. The image digest carries both attestation types, so pass `--predicate-type` to verify the SBOM specifically:
...
A successful verification confirms the SBOM was produced and signed by the same `release-cut.yml` run that built the image. To inspect the SBOM contents rather than just verify the signature, a human-readable copy is also attached to the GitHub release as `drydock-${TAG}.image.spdx.json` — but that file is a convenience copy, not itself a security control. Trust the attestation verification above (or extract the SBOM from the attestation bundle) before relying on package-list contents for vulnerability triage.
