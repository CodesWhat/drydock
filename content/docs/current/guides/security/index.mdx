...
Drydock includes vulnerability scanning, image signature verification, SBOM generation, report export, and multiple Docker socket isolation options. This guide walks through each layer so you can build up a defense-in-depth deployment.
...
services:
  socket-proxy:
    ...
    environment:
      ...
      - IMAGES=1
      - EVENTS=1
      - INFO=1         # required for server-name detection (DD_SERVER_NAME fallback)
...
The socket proxy can use infrastructure self-update because its own container mounts the Docker socket directly. The helper uses that target-container bind for the replacement; Drydock itself does not need a direct socket mount for this proxy-based setup.
...
For remote Docker APIs that require basic, bearer, or OIDC auth, use HTTPS or mTLS. Drydock fails closed for remote watcher credentials on plain HTTP unless `AUTH_INSECURE=true` is explicitly configured for a trusted legacy network.
...
Since v1.4.0, authentication is required by default. Fresh installs require authentication configuration before access unless anonymous access is explicitly confirmed with `DD_ANONYMOUS_AUTH_CONFIRM=true`. Upgrades behave identically to fresh installs — an existing anonymous instance also fails closed — every API request returns `401` — unless `DD_ANONYMOUS_AUTH_CONFIRM=true` is set or authentication is configured.
...
```bash
printf '%s' 'your-password' | argon2 "$(openssl rand -base64 32)" -id -m 16 -t 3 -p 4 -l 64 -e
```
...
environment:
  ...
  - DD_AUTH_OIDC_SSO_DISCOVERY=https://auth.example.com/.well-known/openid-configuration
  - DD_AUTH_OIDC_SSO_CLIENTID=drydock
  - DD_AUTH_OIDC_SSO_CLIENTSECRET=xxxxx
```
...
Drydock trims trailing whitespace from file-sourced secrets and warns if a Unix secret file is readable by group or others. Keep secret files at `0600` permissions where possible:
...
environment:
  ...
  - DD_NOTIFICATION_SMTP_GMAIL_FROM=drydock@example.com
  - DD_NOTIFICATION_SMTP_GMAIL_TO=security-team@example.com
  - DD_NOTIFICATION_SMTP_GMAIL_SECURITYMODE=digest
```
...
| --- | --- | --- |
| Socket isolation | Using socket proxy, remote mTLS, rootless Docker, or rootless Podman — not a direct host Docker socket mount | [Watchers](/docs/configuration/watchers#docker-socket-security) |
| Remote watcher auth | `DD_WATCHER_{name}_AUTH_*` credentials stay on HTTPS/mTLS watchers; `AUTH_INSECURE=true` is avoided except as a temporary trusted-network exception | [Remote watcher auth](/docs/configuration/watchers#watch-a-remote-docker-host-behind-https-with-oidc-auth) |
| Podman | Direct rootless Podman socket mounts are preferred first; socket proxy/TCP Podman paths depend on Podman's Docker-compatible HTTP API | [Podman Quick Start](/docs/configuration/watchers#podman-quick-start) |
...
| Update authority | Docker triggers and socket-proxy `POST`/`NETWORKS` permissions are enabled only where Drydock should recreate containers | [Docker trigger](/docs/configuration/triggers/docker) |
