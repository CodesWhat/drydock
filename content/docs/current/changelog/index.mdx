...
- **OIDC discovery fails on HTTP IdP URLs** — openid-client v6 enforces HTTPS by default for all OIDC requests. Users running IdPs behind reverse proxies or on private networks with HTTP discovery URLs got "only requests to HTTPS are allowed" errors. Now temporarily allows `http://` discovery in compatibility mode (with warnings), supports explicit override via `DD_OIDC_ALLOW_HTTP=true|false`, and surfaces a dismissible UI banner when HTTP discovery is detected.
