...
- **Canonical update-lifecycle event stream ([#289](https://github.com/CodesWhat/drydock/issues/289), [#290](https://github.com/CodesWhat/drydock/issues/290)).** `dd:update-applied`, `dd:update-failed`, and the new `dd:batch-update-completed` are now broadcast to all connected browsers as first-class SSE events with stable payloads keyed by `operationId` and `batchId`. Bulk updates emit one summary event when the last operation in the batch reaches terminal so UI surfaces can show "Updated N containers in {group}" instead of N independent toasts. Every browser tab, the notification bell, the in-page toast queue, and external notification providers all consume the same canonical events.
- **SSE event replay buffer (Phase 2-A).** Bounded ring buffer with size + age TTL keyed by monotonically assigned event IDs. On reconnect, the browser's `EventSource` sends `Last-Event-ID` per the SSE spec and the handler replays any buffered events newer than the cursor. Closes the silent-toast-loss window (browser disconnects during an update window previously dropped any terminal events that fired in between).
- **Agent-managed updates flow through the controller's lifecycle stream (Phase 2-B).** Agents now forward phase events alongside terminals; AgentClient translates each into `insertOperation` / `markOperationTerminal` calls on the controller's store so phase events broadcast to browsers identically to controller-local updates. Agent-applied updates now show toasts, progress counters, and bell entries — previously they were invisible.
- **Pinia event-stream architecture (Phase 3).** Six disjoint UI state sources (`actionInProgress`, `actionPending`, `heldOperations`, `useUpdateBatches.batches`, audit-log-poll bell, imperative click-handler toasts) collapsed into a single `useEventStreamStore` plus derived `useOperationStore`, `useToastStore`, and `useNotificationStore`. Toast emission moves to declarative store watchers — components no longer call `toast.success(...)` imperatively. NotificationBell, ContainersView, and DashboardView migrated to the new stores.
- **Architecture lint guard.** A test file (`ui/tests/architecture/eventStreamGuard.spec.ts`) fails the gate if `new EventSource` appears outside `ui/src/stores/eventStream.ts`. Single-source-of-truth is now enforced, no allowlist.

### Fixed

- **[#340](https://github.com/CodesWhat/drydock/discussions/340) - Self-update no longer preserves stale Drydock version metadata.** Replacement containers now inherit the new image's `DD_VERSION` and `org.opencontainers.image.version` values instead of carrying version metadata from the previous Drydock image. Operator-supplied environment variables and labels remain preserved.
- **[#290](https://github.com/CodesWhat/drydock/issues/290) — "Updated successfully" notification not delivered for Ungrouped Update All.** The completion toast was gated on a start-side display hold that never got created when the start API call returned with no accepted operations (e.g., guard fired silently); the row would vanish without any toast and the bell stayed at 9+. The fail-safe completion path now subscribes directly to `dd:update-applied` and fires regardless of hold state, deduped by `operationId` against the legacy hold-driven path. The silent early-return guard at `useContainerActions.ts` now warns "Update already in progress for some containers in this group" instead of swallowing the click.
- **[#289](https://github.com/CodesWhat/drydock/issues/289) — Stack progress counter stuck at "0 of 2 done"; singular completion toast for plural updates.** `frozenTotal` was a click-time snapshot and the batch was cleared the instant `activeCount` hit 0, so the counter never advanced past 0. Live `succeededCount` / `failedCount` are now driven by terminal SSE events with a 1500ms display hold at "Y of Y done" before clearing, and a single batch summary toast (success / warning for partial / error for all-failed) replaces the N independent per-container toasts when an operation has a non-null `batchId`.
- **Notification bell badge stuck at "9+" forever.** `lastSeen` previously only advanced on explicit Mark All Read clicks, capped at 9, so once a user accumulated 10+ unread entries the badge never decreased. `lastSeen` now advances on every panel-close path (button toggle, click-outside, navigate-to-entry, open-audit-log). The bell also subscribes to `dd:sse-update-operation-changed` and `dd:sse-update-applied` so panel content refreshes as soon as those events fire.
- **[#321](https://github.com/CodesWhat/drydock/discussions/321) — Floating Docker tags now compare as same-tag digest updates by default.** Mutable aliases such as `latest`, `stable`, `v3`, `16-alpine`, and `10.6` now enable digest watching even on Docker Hub unless explicitly disabled, so Drydock detects the same updates Portainer reports for tags that move in place. Strict tag-family mode also treats semver-shaped floating aliases as digest-only by default, preventing `postgres:16-alpine` from being proposed as a tag update to `17-alpine`; set `dd.tag.family=loose` to opt back into cross-tag semver updates.
- **`dd.registry.lookup.image` label no longer corrupts deploy identity ([fixes #336](https://github.com/CodesWhat/drydock/issues/336)).** The lookup label redirects registry queries to a different image (e.g. a private mirror running `myreg/nextcloud` looking up tags from `library/nextcloud`), but `normalizeContainer` was overwriting the container's own `image.name` and `image.registry.url` with the substituted values so recreates deployed the lookup target instead of the original mirror image. Deploy identity is now preserved; the substitution is applied only at registry-query time.
- **Password-manager autofill restored on login form ([fixes #335](https://github.com/CodesWhat/drydock/issues/335)).** Username/password `name` and `id` attributes were dropped during the v1.5 plain-HTML rewrite, breaking credential managers that rely on them (e.g. Dashlane in Chrome). Both attributes are restored.
- **`security-scan-skipped` audit row now fires when the gate is disabled via `DD_SECURITY_GATE_MODE=off`.** Previously the audit row only recorded when `dd.security.gate=off` was set per-container; global env-var suppression produced no audit trail.

### Added

- **Chinese (Simplified) UI** — First non-English locale bundled in drydock. 14 namespace JSON files under `ui/src/locales/zh/` cover the full interface (~1,100+ strings), contributed by [TianMiao](https://github.com/TianMiao) ([PR #331](https://github.com/CodesWhat/drydock/discussions/331)). Switch languages in **Config > Appearance**. The i18n auto-discovery framework picks up the locale automatically; Crowdin integration for community contributions is planned for v1.8.0.
- **Multi-select event-type filter in audit log ([discussion #332](https://github.com/CodesWhat/drydock/discussions/332)).** The Audit Log event-type filter is now a checkbox dropdown so operators can view any combination of event categories (e.g. `update-applied` + `update-failed`) in a single query. Back-compat: `?action=` bookmark URLs continue to work as a single-element selection.

### Security

- **Credential redaction expanded to `x-registry-auth`, `*-token`, and `api-key` fields.** `update-failed` SSE error payloads now redact registry auth tokens, API keys, and OAuth bearer strings embedded in structured error objects — not only `Authorization` header values. Operator-visible diagnostics no longer leak secrets from registry interactions.

## [1.5.0-rc.16] — 2026-04-28

### Added

- **i18n framework migration ([refs #329](https://github.com/CodesWhat/drydock/issues/329)).** Bulk vue-i18n migration of ~73 `.vue` files into per-namespace JSON catalogs under `ui/src/locales/en/` (eight namespaces: `dashboardView`, `containersView`, `configView`, `agentsView`, `listViews`, `containerComponents`, `appShell`, `sharedComponents` — auto-loaded by an `import.meta.glob` in `boot/i18n.ts` so new namespaces are picked up automatically). Plus AuthView, LoginView, LogsView, and ContainerLogsView migrated as part of the same pass. Static label arrays (`tableColumns`, `settingsTabs`, etc.) wrapped in `computed()` so they react to runtime locale changes; pure leaf components with no translatable strings intentionally left untouched (no useless `useI18n()` imports). Coverage by batch: dashboard view + 6 widgets (~55 strings), ContainersView shell (~8), ConfigView + 4 tabs (~65 — `DD_*` env names left untranslated), AgentsView + 3 detail tabs (~65), 7 list views (Audit/Notifications/Security/Registries/Servers/Triggers/Watchers, ~150), 19 container components (~250), app-shell — AppLayout/AppToast/NotificationBell/ThemeToggle/AnnouncementBanner/AppLogViewer (~76), and 17 shared/data components (~27). Foundation for the v1.8.0 Crowdin integration; English was the only catalog at rc.16 — Chinese (Simplified) was added in rc.17.
...
- **Floating tag detection and UI indicator** — New `tagPrecision` classifier (`specific` | `floating`) detects mutable version aliases (e.g. `v3`, `1.4`) and auto-enables digest watching for floating tags. Container detail views show a caution badge when a floating tag is detected without digest watching enabled. ([Discussion #178](https://github.com/CodesWhat/drydock/discussions/178))
