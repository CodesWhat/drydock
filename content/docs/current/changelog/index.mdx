...
- **Multi-server notification identification (#283)** — Notifications automatically include a `[server-name]` prefix when agents are registered, identifying which server (controller or agent) each update comes from. Controller name configurable via `DD_SERVER_NAME` (defaults to `os.hostname()`). Custom templates can use `container.notificationServerName` and `container.notificationAgentPrefix`. ([Discussion #283](https://github.com/CodesWhat/drydock/discussions/283))
- **Infrastructure container update mode** — New `dd.update.mode=infrastructure` label for socket proxy and similar infrastructure containers. Routes updates through a helper-container path that bypasses the proxy, enabling safe in-place updates without severing Drydock's Docker API connection. ([Discussion #285](https://github.com/CodesWhat/drydock/discussions/285))
- **Docker client auto-recreation on reconnect** — When the Docker event stream reconnects (e.g. after a socket proxy restart), Drydock now recreates the Dockerode client with a fresh connection instead of reusing the stale one.
- **Deferred reconciliation for stuck containers** — When a container update fails and rollback also fails due to a lost Docker API connection, Drydock now schedules a delayed recovery attempt instead of leaving containers in an inconsistent state.
...
- **Container filter labels** — Default dropdown options renamed for clarity: "Status" → "All Statuses", "Host" → "All Hosts", etc. ([#247](https://github.com/CodesWhat/drydock/discussions/247))

### Fixed

- **#248** — Update operation state survives container ID change during replacement updates.
- **#241** — Temporary rollback containers hidden from container list API responses.
- **#253** — `threshold=all` honored before rejecting `updateKind=unknown` in notification dispatch.
- **#256** — Update operation state disambiguated by container ID instead of name.
- **#257** — Non-dashboard views refresh on SSE reconnect.
- **#258** — Edge-to-edge scroll on DataViewLayout for mobile touch.
- **#217** — Content-aware mode threshold for Host Status widget.
- **#262** — Registry log calls use component child logger with image name context.
- **#269** — SBOM download button was silently failing unless the "View SBOM" toggle was active.
- **#255** — Container event handler schedules refresh for unknown containers.
