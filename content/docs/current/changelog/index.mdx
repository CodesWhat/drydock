...
- **Container state not updated when started/stopped externally** — Containers started or stopped outside Drydock (via Portainer, Docker CLI, etc.) could show stale status until the next Docker event. The hourly cron poll now reconciles container status from Docker's `listContainers` response.
...
[1.1.1]: https://github.com/CodesWhat/drydock/compare/v1.1.0...v1.1.1
