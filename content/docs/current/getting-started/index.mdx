...
The threshold controls which range of change magnitudes are allowed through, not a minimum severity floor. For example, `THRESHOLD=major` fires on every semver bump (major, minor, or patch), while `THRESHOLD=minor` fires on minor and smaller changes but skips major version jumps.

```yaml
environment:
  - DD_NOTIFICATION_SLACK_ALERTS_TOKEN=xoxb-xxxxx
  - DD_NOTIFICATION_SLACK_ALERTS_CHANNEL=#docker-updates
  - DD_NOTIFICATION_SLACK_ALERTS_THRESHOLD=minor  # skip digest-only and major bumps
```
