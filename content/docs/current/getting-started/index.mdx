...
```yaml
environment:
  - DD_NOTIFICATION_SLACK_ALERTS_TOKEN=xoxb-xxxxx
  - DD_NOTIFICATION_SLACK_ALERTS_CHANNEL=#docker-updates
```
...
```yaml
environment:
  - DD_NOTIFICATION_DISCORD_ALERTS_URL=https://discord.com/api/webhooks/xxxxx
```
</Tab>
<Tab value="Email (SMTP)">
```yaml
environment:
  - DD_NOTIFICATION_SMTP_ALERTS_HOST=smtp.gmail.com
  - DD_NOTIFICATION_SMTP_ALERTS_PORT=465
  - DD_NOTIFICATION_SMTP_ALERTS_FROM=drydock@example.com
  - DD_NOTIFICATION_SMTP_ALERTS_TO=admin@example.com
  - DD_NOTIFICATION_SMTP_ALERTS_USER=drydock@example.com
  - DD_NOTIFICATION_SMTP_ALERTS_PASS=xxxxx
```
</Tab>
<Tab value="ntfy">
```yaml
environment:
  - DD_NOTIFICATION_NTFY_ALERTS_TOPIC=drydock-updates
```

Uses `https://ntfy.sh` by default. Set `DD_NOTIFICATION_NTFY_ALERTS_URL` for a self-hosted instance.
</Tab>
<Tab value="Other">
Drydock supports 21 trigger integrations — 17 notification channels plus 4 action providers (Docker, Docker Compose, Portainer, Command). See the [Triggers configuration](/docs/configuration/triggers) page for all options including Telegram, Teams, Matrix, Gotify, Pushover, Apprise, IFTTT, HTTP webhooks, Kafka, MQTT, and more.
</Tab>
</Tabs>
...
| `major` | Any semver change, including prerelease, or a digest-only update |
| `minor` | Minor, patch, prerelease, or digest-only updates |
| `patch` | Patch, prerelease, or digest-only updates |
| `digest` | Digest-only updates |

The threshold controls which range of change magnitudes are allowed through, not a minimum severity floor. For example, `THRESHOLD=major` fires on every semver bump (major, minor, or patch), while `THRESHOLD=minor` fires on minor and smaller changes but skips major version jumps. Digest-only updates pass through `major`, `minor`, and `patch` too, since a plain threshold only caps the semver magnitude and does not filter by update kind; append `-no-digest` (e.g. `minor-no-digest`) to exclude digest-only updates.

```yaml
environment:
  - DD_NOTIFICATION_SLACK_ALERTS_TOKEN=xoxb-xxxxx
  - DD_NOTIFICATION_SLACK_ALERTS_CHANNEL=#docker-updates
  - DD_NOTIFICATION_SLACK_ALERTS_THRESHOLD=minor  # skip major bumps (digest-only updates still fire)
```
...
```yaml
environment:
  - DD_ACTION_DOCKER_LOCAL_PRUNE=true      # clean up old images
  - DD_ACTION_DOCKER_LOCAL_DRYRUN=true     # preview only — remove when ready
```
...
```yaml
environment:
  - DD_ACTION_DOCKERCOMPOSE_MYSTACK_FILE=/compose/my-stack.yml
  - DD_ACTION_DOCKERCOMPOSE_MYSTACK_DRYRUN=true  # preview only
volumes:
  - /path/to/my-stack/compose.yml:/compose/my-stack.yml  # must be writable
```

The compose trigger patches the image tag in your compose file, then refreshes the service through the Docker Engine API. The `docker compose` / `docker-compose` CLI is not required inside the Drydock container.
...
environment:
  ...
  - DD_ACTION_DOCKERCOMPOSE_STACK_FILE=/compose/stack.yml
volumes:
...
```yaml
environment:
  - DD_ACTION_DOCKER_LOCAL_BACKUPCOUNT=3
```
...
Updates detected outside the window are queued and applied when it opens, for containers on local watchers; containers watched by a remote agent are not deferred by the controller yet, even though the update dialog shows the window as closed for them. Drydock keeps scanning on its ordinary schedule meanwhile, so new containers, container state and update notifications are not held back; set `DD_WATCHER_LOCAL_MAINTENANCE_WINDOW_SCOPE=scan` if you want the window to stop the scan too. The window only gates what drydock does on its own. You can still update any container manually at any time, and the confirm dialog will note the window is closed and offer **Update anyway**.
...
```yaml
labels:
  - dd.hook.pre=/usr/local/bin/pre-update-backup.sh
...
```yaml
environment:
  - DD_AGENT_REMOTE1_HOST=<agent-ip>
  - DD_AGENT_REMOTE1_SECRET=a-secure-shared-token
  - DD_AGENT_ALLOW_INSECURE_SECRET=true  # plain-HTTP LAN only; use TLS in production
  # - DD_AGENT_REMOTE1_PORT=3000  # only needed if the agent listens on a non-default port
```
...
services:
  drydock-agent:
    ...
    command: --agent
    ports:
      - "3000:3000"  # agent API — the controller connects to this port
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
    environment:
      - DD_AGENT_SECRET=a-secure-shared-token
      - DD_WATCHER_LOCAL_SOCKET=/var/run/docker.sock
```

Agents forward container state to the controller. The controller connects outbound to agents; agents do not call back to the controller. Notification triggers (Slack, SMTP, etc.) run on the controller; Docker update triggers run on the agent.
