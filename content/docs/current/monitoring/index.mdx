...
dd_legacy_input_total{source="env",key="DD_TRIGGER_SLACK_LOCAL_URL"} 1
...
`dd_legacy_input_total` tracks active compatibility inputs such as deprecated `DD_TRIGGER_*` variables, plus historical observations retained for the lifetime of the process. Removed `WUD_*` variables and `wud.*` labels no longer increment it because they are ignored. No data is sent to external services.
