...
If the application is healthy, the HTTP response status code is `200`. During startup, before authentication strategies are ready, drydock returns `503` with a startup readiness payload.
