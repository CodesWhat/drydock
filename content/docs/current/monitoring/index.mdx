...
If the application is healthy, the HTTP response status code is `200`. Before at least one authentication strategy is registered, drydock returns `503` with a startup readiness payload. This is usually brief, resolving as soon as auth strategies finish registering during startup, but it is not purely a startup phase: if the instance ends up with zero registered auth strategies (for example, a fresh install with no `DD_AUTH_*` configuration and no `DD_ANONYMOUS_AUTH_CONFIRM=true`), the readiness check never becomes true and `/health` stays `503` indefinitely. This is a deliberate fail-closed design, not a bug: an instance with no way to authenticate is not considered healthy.
...
<Callout type="warn">**Removed in v1.7.0:** `curl` is no longer included in the Docker image. Custom `healthcheck:` overrides in compose files that shell out to curl (e.g. `test: curl --fail http://localhost:3000/health`) will fail. When Drydock can inspect its own container through the local Docker socket, it shows a UI deprecation banner and logs a startup warning naming the container if it detects a curl-based override. Remove custom `healthcheck:` blocks from your drydock service to use the built-in probe, or point the override at `/bin/healthcheck` directly (see below).</Callout>
...
The doubled `$` in `$${DD_SERVER_PORT:-3000}` is compose escaping: a single `$` gets expanded by compose from the host environment before the container ever starts, so a port set only in the service's `environment:` block would silently resolve to `3000`. The doubled form defers expansion to the container shell, which the string-form `test:` runs under `CMD-SHELL`.
...
2. **Registered authenticator chain fallback** — When no `DD_SERVER_METRICS_TOKEN` is set and `DD_SERVER_METRICS_AUTH` is not `false`, the endpoint falls back to the registered authenticator chain used by the rest of the API, including the session authenticator plus configured providers such as Basic or OIDC. Because this mode accepts a user password, failed attempts are capped at 100 per IP per 15 minutes. Only failures count, so a scrape that authenticates is never charged against the budget however often it runs.
...
# HELP dd_legacy_input_total Total number of legacy compatibility inputs observed
# TYPE dd_legacy_input_total counter
dd_legacy_input_total{source="label",key="dd.trigger.include"} 1
...
# HELP dd_watcher_maintenance_deferred_updates_total The number of automatic updates deferred by a maintenance window, one per container per dispatch evaluation, including containers deferred because a dependency was
# TYPE dd_watcher_maintenance_deferred_updates_total counter
dd_watcher_maintenance_deferred_updates_total{type="docker",name="local"} 0
...
The two maintenance-window counters measure different things and are not interchangeable. `dd_watcher_maintenance_skip_total` counts whole scan cycles the window skipped, so it only moves on a watcher running `MAINTENANCE_WINDOW_SCOPE=scan`; under the default `install` scope no scan is ever skipped and it stays at zero. `dd_watcher_maintenance_deferred_updates_total` counts automatic updates held back for the window, one sample per container per dispatch evaluation, so a batch that defers fifty containers adds fifty and the next scan that still finds them deferred adds fifty more. A container held back because something it depends on was deferred is counted too, labelled by its own watcher.

`dd_legacy_input_total` tracks active compatibility inputs such as the deprecated `dd.trigger.include` / `dd.trigger.exclude` container labels, plus historical observations retained for the lifetime of the process. The `DD_TRIGGER_*` environment variable prefix was removed in v1.7.0 and now fails startup outright, so it can no longer increment this counter — see [Deprecation Schedule](/docs/deprecations#legacy-trigger-prefix). Removed `WUD_*` variables and `wud.*` labels no longer increment it because they are ignored. No data is sent to external services.
...
- `drydock_auth_api_key_failure_total` counter, every rejected API key authentication attempt, including those the audit-entry throttle (at most one entry per source key per minute) suppresses from the audit trail
...
groups:
  - name: drydock-auth
    rules:
      ...
      # API key authentication abuse, more than 10 rejected key attempts in 5 minutes
      - alert: DrydockApiKeyAuthFailureRate
        expr: increase(drydock_auth_api_key_failure_total[5m]) > 10
        for: 1m
        labels:
          severity: warning
        annotations:
          summary: "High API key authentication failure rate on Drydock"
          description: >-
            {{ $value | printf "%.0f" }} rejected API key authentication attempts in the last 5
            minutes. This counter includes attempts the audit-entry throttle suppresses, so it
            can be elevated even when the audit trail looks quiet.
```
...
| API key authentication failure rate | Time series | `rate(drydock_auth_api_key_failure_total[5m])` |
...
| `DD_AUTH_LOCKOUT_PRUNE_INTERVAL_MS` | `60000` (1 min) | very high login volume makes pruning too frequent | expired lockout state lingers too long for your memory budget |
| `DD_AUTH_LOCKOUT_MAX_TRACKED_IDENTITIES` | `5000` | many unique client IPs exist (e.g. egress across many subnets) | memory pressure from lockout state storage is a concern |
| `DD_AUTH_MAX_CONCURRENT_LOGIN_ATTEMPTS` | `2` | legitimate login bursts receive `429` responses | password hashing causes excessive CPU or memory pressure |
...
- `update-available` on first detection, on a change of the update target, or on re-detection after the update resolves or the container is removed/recreated. Unchanged updates are never re-asserted just because time has passed.
...
It can be imported into any Grafana instance. Pick an existing data source.
