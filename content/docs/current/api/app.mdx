...
The dump includes runtime metadata, component state (watchers, registries, triggers, agents, containers), Docker API diagnostics, MQTT Home Assistant sensor definitions, recent Docker events, store statistics, and `DD_*` environment variable names. In unreleased v1.8 builds, store statistics report SQLite table row counts under the compatibility field `collections` (each entry has `name` and `documents`). The aggregate `serializedBytes` is SQLite's `page_count` multiplied by `page_size`, not JSON serialization size or per-table byte attribution. Internal migration and store-metadata tables are excluded from the row counts; raw stored rows are not included.
...
| `store` | SQLite table row counts, aggregate database page size as `serializedBytes`, and store paths; no per-table byte attribution |
