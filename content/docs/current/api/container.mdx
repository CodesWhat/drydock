...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `touch` | boolean | `true` | When `false`, returns cached snapshots without spinning up per-container Docker stats streams. Use `?touch=false` for read-only summary queries (dashboard) to avoid warming streams unnecessarily. |
...
Returns a fleet-wide CPU and memory snapshot aggregated across all locally-monitored containers. Updated on a configurable polling interval (default 10 s). Use this endpoint for read-only dashboard queries; unlike the per-container stats endpoints, it does not spin up Docker stats streams.
