...
Returns 200 with `accepted` and `rejected` arrays, 403 if container actions are disabled, 404 if the root container is not found, or 500 if the update requests cannot be accepted.
