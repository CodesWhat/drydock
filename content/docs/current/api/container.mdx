...
The policy gate is a per-container setting that suppresses an update until the image has aged past a minimum threshold. Configure it from the UI (Update policy menu) or via the API — there is no `dd.*` container label for this.
