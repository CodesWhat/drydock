...
This operation triggers a vulnerability scan on all watched containers server-side. Streams per-container progress over the SSE channel and emits a single `security-scan-cycle-complete` event when finished. A prematurely closed, incomplete request stops queued scans. Once the request is complete and accepted, scanning continues after the `202` response; closing the client or stopping its progress display does not cancel the accepted work.
