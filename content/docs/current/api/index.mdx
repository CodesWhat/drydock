...
| --- | --- | --- |
| `error` | string | Human-readable error message (legacy field, same as `message`) |
| `code` | string | Machine-readable error code (e.g. `NOT_FOUND`, `UNAUTHORIZED`, `CONTAINER_NOT_FOUND`) |
| `message` | string | Human-readable error message |
| `details` | object | Optional contextual metadata (present only when additional context is available) |

Error codes are derived from the HTTP status and error message. Common codes include `BAD_REQUEST`, `UNAUTHORIZED`, `FORBIDDEN`, `NOT_FOUND`, `CONTAINER_NOT_FOUND`, `TOO_MANY_REQUESTS`, and `INTERNAL_SERVER_ERROR`.
