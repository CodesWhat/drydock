...
The legacy `{ strategies, warnings }` response shape at `GET /auth/strategies` was removed in v1.8.0; the route is no longer mounted and now falls through to the SPA shell. Use `GET /api/v1/auth/status` instead; see [Deprecations](/docs/deprecations#legacy-auth-strategies-shape).
