...
Scope: `read`. Returns 200 with the pending/deferred/decided counts used for the nav badge and dashboard tile.
...
Scope: `read`. Returns 200 with `{ approval, eligibility, holdReasons }` (`holdReasons` is empty when the approval's container no longer exists), or 404 if the approval does not exist.
