...
If the boot ID does not match the server's current boot (server restarted since the last connection), the server cannot replay events from before the restart. The client should treat a reconnect after a reboot as a full re-sync.
