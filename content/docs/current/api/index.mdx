...
| `/api/audit` | `GET` | Audit trail entries (paginated, filterable by action/container/date) |
| `/api/notifications` | `GET`, `PATCH` | Notification rules (per-event enable/disable and trigger assignments) |
| `/api/settings` | `GET`, `PUT` | UI settings (internetless mode) |
| `/api/server` | `GET` | Server configuration and compatibility info |
