...
Both paths return identical responses, except when `DD_COMPAT_WUDCARD=true` reshapes three of the four whitelisted [wud-card compatibility](/docs/configuration/server#wud-card-compatibility) endpoints into a bare-array shape. Third-party integrations should migrate to `/api/v1/*` at their convenience.
