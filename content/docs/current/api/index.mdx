...
The four whitelisted [wud-card compatibility](/docs/configuration/server#wud-card-compatibility) endpoints keep responding at `/api/*` when `DD_COMPAT_WUDCARD=true` (three reshaped into a bare-array shape, the fourth unmodified), served by their own router rather than by falling through to the removed alias. `GET /api/auth/status` is also unaffected — unconditionally, not behind any flag — because it's registered directly on the app rather than through the removed alias router; see [Authentication routes](#authentication-routes) below. Everything else under `/api/*` now returns `410 Gone`, including the former `GET /api/auth/methods` alias, removed in v1.7.0; use `/api/v1/*`. See [Deprecations](/docs/deprecations#unversioned-api-paths) for details.
...
When [authentication](/docs/configuration/authentications) is enabled, all API endpoints require either a valid session (cookie-based via Basic or OIDC login) or a [scoped API key](/docs/configuration/authentications/api-keys).
...
A scoped API key is sent as `Authorization: Bearer ddk_<keyId>_<secret>`. It authenticates a single request, never creates a session, and therefore isn't subject to the same-origin CSRF checks that apply to cookie auth. Each route is reachable at exactly one scope; a key without it gets `403`. A handful of routes are session-only and unreachable with a key at any scope. Manage keys through the [API Keys API](/docs/api/api-keys).
...
| `/api/v1/dependency-groups/:rootId/update` | `POST` | `X-DD-Confirm-Action: dependency-group-update` |
| `/api/v1/api-keys/:keyId` | `DELETE` | `X-DD-Confirm-Action: api-key-revoke` |
...
| [API Keys](/docs/api/api-keys) | `GET /api/v1/api-keys`, `POST`, `DELETE /:keyId` | Scoped API key management (`api-keys:manage`) |
...
| `/api/v1/approvals` | `GET`, `GET /summary`, `GET /:id`, `POST /:id/approve`, `POST /:id/reject`, `POST /:id/defer` | [Approvals](#approvals) (pending-approval queue: list, count, detail, and decide) |
...
| `/api/v1/portwing/keys` | `GET`, `POST` | Stable Portwing edge-agent key registry, enabled by default (`DD_EXPERIMENTAL_PORTWING=false` is an emergency disable) |
| `/api/v1/portwing/keys/:keyId` | `DELETE` | Revoke a Portwing Ed25519 public key and disconnect live sessions authenticated with it |
...
Notification rules control which events trigger notifications, which triggers receive them, the in-app bell filters, and optional per-provider message templates. Eight default rules are created automatically.
...
The pending-approval queue holds container updates waiting on an operator decision. Approve dispatches the update through the same admission path as the container panel's Update button, reject writes the container's `skipTags`/`skipDigests` policy, and defer writes a snooze; all three record an audit entry (`update-approved`, `update-rejected`, `update-deferred`) and emit an SSE event (see [Event types](#event-types)).

### List approvals

```bash
curl "http://drydock:3000/api/v1/approvals?status=pending&limit=25"
```

Scope: `read`. Defaults to `status=pending`. Supports `status` (`pending`, `deferred`, `decided`, or `all`), `containerId`, `agent`, `semverDiff` (`major`, `minor`, `patch`, `prerelease`, or `unknown`), a free-text `q` filter (max 200 characters), and `limit`/`offset` pagination. Returns 200 with `{ data, total, limit, offset, hasMore }`, or 400 if a filter value is invalid.
...
Scope: `read`. Returns 200 with `{ pending, deferred, decidedToday }` — `decidedToday` counts decisions since UTC midnight, not the running total of decided approvals — used for the nav badge and dashboard tile.

### Get a single approval

```bash
curl http://drydock:3000/api/v1/approvals/approval-id
```

Scope: `read`. Returns 200 with `{ approval, eligibility, holdReasons }` (`eligibility` is omitted and `holdReasons` is empty when the approval's container no longer exists), or 404 if the approval does not exist.

### Approve a queued update

```bash
curl -X POST http://drydock:3000/api/v1/approvals/approval-id/approve \
  -H 'Content-Type: application/json' \
  -d '{"note": "cleared with the on-call team"}'
```

Scope: `containers:update`. Body accepts an optional `note` (string, max 500 characters); no other properties are allowed. Dispatches the update exactly as `POST /containers/:id/update` would, so it returns the same outcomes that endpoint returns, including the `updateMode: notify` 409. Also returns 403 if container actions are disabled, 404 if the approval or its container no longer exists, or 409 if the approval was already decided or its candidate was superseded by a newer sighting.

### Reject a queued update

```bash
curl -X POST http://drydock:3000/api/v1/approvals/approval-id/reject \
  -H 'Content-Type: application/json' \
  -d '{"note": "holding for the next patch release"}'
```

Scope: `containers:update`. Body accepts an optional `note`, same rules as approve. Writes the container's skip policy and returns 200 with `{ approval }`. Same 404/409 conditions as approve.

### Defer a queued update

```bash
curl -X POST http://drydock:3000/api/v1/approvals/approval-id/defer \
  -H 'Content-Type: application/json' \
  -d '{"days": 14, "note": "checking with the vendor first"}'
```

Scope: `containers:update`. Body accepts `until` (an RFC 3339 date-time in the future, at most 365 days out) or `days` (integer, 1 to 365, defaults to 7), plus an optional `note`. You don't need to send both; if both are present, `until` takes precedence. Writes the container's snooze to the same instant and returns 200 with `{ approval }`. Returns 400 if `until`/`days` is invalid, plus the same 404/409 conditions as approve.
...
`update-available`, `update-applied`, `update-applied-dryrun`, `update-failed`, `notification-delivery-failed`, `container-update`, `security-alert`, `security-scan-skipped`, `scanner-asset-pull-started`, `scanner-asset-pull-succeeded`, `scanner-asset-pull-failed`, `scanner-asset-warm-started`, `scanner-asset-warm-succeeded`, `scanner-asset-warm-failed`, `agent-disconnect`, `container-unhealthy`, `maturity-cleared`, `container-added`, `container-removed`, `rollback`, `preview`, `container-start`, `container-stop`, `container-restart`, `webhook-watch`, `webhook-watch-container`, `webhook-update`, `hook-configured`, `hook-pre-success`, `hook-pre-failed`, `hook-post-success`, `hook-post-failed`, `auto-rollback`, `auth-login`, `env-reveal`, `debug-dump`, `auto-update-blocked`, `update-policy-override-set`, `update-policy-override-cleared`, `update-approved`, `update-rejected`, `update-deferred`, `mqtt-command-update`, `api-key-created`, `api-key-revoked`, `api-key-auth-failed`
...
| `dd:update-operation-changed` | `{ "operationId": "...", "containerName": "...", "triggerName": "...", "status": "...", "phase": "...", "queuePosition": int, "queueTotal": int }` | An in-flight update operation's status or queue position changed. `triggerName` is optional. |
...
| `dd:approval-created` | `{ "id": "...", "containerId": "...", "containerName": "...", "decision": "pending", "pendingCount": int }` | A container update entered the pending-approval queue |
| `dd:approval-decided` | `{ "id": "...", "containerId": "...", "containerName": "...", "decision": "...", "pendingCount": int }` | An operator approved, rejected, or deferred a queued update; `decision` is `approved`, `rejected`, or `deferred` |
| `dd:approval-resolved` | `{ "id": "...", "containerId": "...", "containerName": "...", "decision": "...", "pendingCount": int }` | A queued row left the queue without an operator decision, for example its candidate was superseded or the container was removed |
...
curl -X PATCH http://drydock:3000/api/v1/preferences \
  -H 'Content-Type: application/json' \
  -d '{"apiVersion": 1, "schemaVersion": 12, "preferences": {"theme": {"family": "dracula"}, "appearance": {"fontSize": 1.1}}}'
...
Returns server-derived container groups. Group priority: `dd.group` > `com.docker.compose.project` > `com.docker.stack.namespace` > ungrouped. Browser-side manual group overrides are UI preferences: they do not mutate Docker labels and are not reflected in this response.
...
The legacy `{ strategies, warnings }` response shape at `GET /auth/strategies` was removed in v1.8.0. The route now answers `410 Gone` regardless of whether the request is authenticated, with a JSON body of `{ "error": "...", "details": { "migration": "/api/v1/auth/status", "docs": "..." } }`. Use `GET /api/v1/auth/status` instead; see [Deprecations](/docs/deprecations#legacy-auth-strategies-shape).

The legacy `GET /api/auth/methods` alias was removed in v1.7.0 and now returns `410 Gone`; see [Deprecations](/docs/deprecations#api-auth-methods).
