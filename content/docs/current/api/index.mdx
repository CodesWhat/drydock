...
Returns 200 with the preferences envelope, 400 if the body is malformed, 401 if there is no session, 403 if the session is anonymous (sync requires a real user identity), or 409 if `apiVersion` does not match the server's supported version.

## Container groups

Returns containers grouped by stack or group label. Group priority: `dd.group` > `wud.group` > `com.docker.compose.project` > `com.docker.stack.namespace` > ungrouped.
