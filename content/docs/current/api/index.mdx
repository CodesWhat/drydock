...
| `/api/v1/images/prune` | `POST` | `X-DD-Confirm-Action: image-prune` |
...
| [Config file](/docs/configuration/config-file#api) | `GET /api/v1/config`, `GET /:section`, `POST /validate`, `POST /reload`, `PUT /:section`; `GET` / `PATCH /editor/watchers`, `/editor/triggers`, `/editor/actions` | Unreleased v1.8 file configuration and source-aware editors; reads require a session, writes require admin |
...
| [Approvals](/docs/api/approvals) | `GET /api/v1/approvals`, `GET /summary`, `GET /:id`, `POST /:id/approve`, `POST /:id/reject`, `POST /:id/defer` | Pending-approval queue: list, count, detail, and decide |
...
| Images | `GET /api/v1/images`, `GET /api/v1/images/prune-preview`, `POST /api/v1/images/prune` | Image inventory across hosts, prune preview, and prune (admin; requires `X-DD-Confirm-Action: image-prune`) |
...
| `dd:scan-started` | `{ "containerId": "...", "cycleId": "..." }` | A security scan started for a container; `cycleId` is present for bulk scans and optional otherwise |
| `dd:scan-completed` | `{ "containerId": "...", "status": "...", "cycleId": "..." }` | A security scan finished; `cycleId` is present for bulk scans and optional otherwise. Bulk requests with `requestId` also include that ID, cumulative `completedCount`, and `scheduledCount`. Filter by request ID, validate the accepted cycle/total, and use the highest count rather than adding events |
