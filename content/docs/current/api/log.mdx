...
| --- | --- | --- |
| `level` | string | Minimum level filter (`trace`, `debug`, `info`, `warn`, `error`, `fatal`); returns entries at or above this severity, not just exact matches |
| `component` | string | Substring filter for component name |
| `tail` | integer | Number of most recent entries to return (default: `100`, independent of the 1,000-entry buffer size) |
...
- The unversioned alias `WS /api/log/stream` was removed in v1.6.0 — the WebSocket upgrade now fails fast with a 410-style rejection instead of connecting. Use the canonical `WS /api/v1/log/stream` path.
...
```json
{
  ...
  "level": "warn",
  "component": "api",
  "msg": "CORS is enabled; review the configured origin"
}
```
