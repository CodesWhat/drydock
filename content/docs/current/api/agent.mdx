...
For agents that cannot accept inbound connections and dial out to drydock instead, see the [Lookout Edge API](/docs/api/lookout).
...
The `logLevel` and `pollInterval` fields correspond to the same-named fields in the [GET /agents](#list-all-agents) response. For edge agents connecting via the lookout protocol see the [Lookout Edge API](/docs/api/lookout).
