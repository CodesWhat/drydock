...
For agents that cannot accept inbound connections and dial out to drydock instead, see the [Portwing Edge API](/docs/api/portwing).
...
The `logLevel` and `pollInterval` fields correspond to the same-named fields in the [GET /agents](#list-all-agents) response. For edge agents connecting via the Portwing protocol, see the [Portwing Edge API](/docs/api/portwing).
...
| `since` | integer | Unix timestamp (milliseconds) -- only entries after this time |
...
Returns the Agent's uptime once at least one watcher is registered. This endpoint is unauthenticated and intended for Docker `HEALTHCHECK` directives.
...
If the agent starts but no watchers register, it returns `503`:
