...
All endpoints require the `X-Dd-Agent-Secret` header, except the unauthenticated `GET /health` check.
...
The `logLevel` and `pollInterval` fields correspond to the same-named fields in the [GET /agents](#list-all-agents) response.
