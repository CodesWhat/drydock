...
The unversioned `/api/*` path is a backward-compatible alias for `/api/v1/*`. Both return identical responses, except when `DD_COMPAT_WUDCARD=true`: three of the four whitelisted [wud-card compatibility](/docs/configuration/server#wud-card-compatibility) endpoints then return WUD's bare-array response shape instead of drydock's envelope (the fourth passes through unmodified). Those endpoints remain mounted at `/api/*` even after this alias is removed.
...
`GET /api/auth/methods` is a legacy, unversioned alias for `GET /auth/strategies`, kept unauthenticated so the login screen can render before a session exists.

| | |
| --- | --- |
| **Deprecated since** | v1.6.0 |
| **Detection** | Log warning on each request: "GET /api/auth/methods is deprecated and will be removed in v1.7.0. Use GET /auth/strategies instead." |
