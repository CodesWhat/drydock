...
`GET /auth/strategies` returned the older `{ strategies, warnings }` response shape. The canonical replacement, `GET /api/v1/auth/status` (also available at `/api/auth/status` and `/auth/status`), returns `{ providers, errors }`. As of v1.8.0, the route is no longer mounted. Unlike `GET /api/auth/methods` above, `/auth/strategies` was never registered under the `/api` mount, so its removal does not fall through to the `410 Gone` tombstone; a request to it now falls through to the SPA catch-all and gets a `200` HTML shell response instead, the same as any other unmatched path.
