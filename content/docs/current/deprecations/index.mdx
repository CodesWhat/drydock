...
Older drydock releases allowed `DD_SERVER_CORS_ENABLED=true` without `DD_SERVER_CORS_ORIGIN` and treated it like `*` (all origins). Current releases fail startup instead; CORS must have an explicit origin.

| | |
| --- | --- |
| **Deprecated since** | v1.4.0 |
| **Detection** | Startup error when CORS is enabled without an explicit origin |
