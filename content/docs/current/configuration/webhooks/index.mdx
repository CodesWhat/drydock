...
| `DD_SERVER_WEBHOOK_TOKEN` | 🔴 | Bearer token for webhook authentication (required when enabled) | Any string | — |

<Callout type="warn">Webhooks are disabled by default. Set `DD_SERVER_WEBHOOK_ENABLED=true` and provide a `DD_SERVER_WEBHOOK_TOKEN` to activate them. Without a token, webhook endpoints will reject all requests.</Callout>
