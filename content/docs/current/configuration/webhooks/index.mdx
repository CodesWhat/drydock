...
<Callout type="warn">Webhooks are disabled by default. Set `DD_SERVER_WEBHOOK_ENABLED=true` and provide either the shared `DD_SERVER_WEBHOOK_TOKEN`, all three endpoint-specific `DD_SERVER_WEBHOOK_TOKENS_*` values, or `DD_SERVER_WEBHOOK_SECRET` for signed registry webhooks. Partial endpoint-specific token sets are rejected at startup.</Callout>
