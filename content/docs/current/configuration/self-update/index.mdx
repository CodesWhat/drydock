...
<Callout type="info">The self-update overlay includes an animated visual indicator and persists through the entire container replacement. The UI polls the status endpoint every 5 seconds and reloads itself once the operation reaches a terminal status. No manual refresh is needed.</Callout>
