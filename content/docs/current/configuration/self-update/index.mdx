...
1. Each SSE client receives a `clientId` and `clientToken` when it connects to the `/api/sse` endpoint (via a `dd:connected` event).
2. When the `dd:self-update` event is broadcast, the server records which client tokens were connected at that moment.
3. Browsers acknowledge the event by sending `POST /api/sse/self-update/:operationId/ack` with their `clientId` and `clientToken`.
