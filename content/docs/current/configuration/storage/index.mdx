...
| `DD_STORE_DB_FILE` | ⚪ | SQLite database filename, stored alongside `DD_STORE_FILE` in the same directory. App metadata, settings, the session secret, UI preferences, Portwing agent keys and name bindings, and API keys with their scopes live here; everything else still lives in `DD_STORE_FILE`, which still has to find your existing data on an upgrade. | filename | `dd.sqlite` |
