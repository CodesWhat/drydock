...
Login sessions persist in their own file next to the main store — `dd-sessions.json` by default, derived from `DD_STORE_FILE` — so mounting `/store` covers both.
...
| `DD_STORE_DB_FILE` | ⚪ | SQLite database filename, stored alongside `DD_STORE_FILE` in the same directory. App metadata, settings, the session secret, UI preferences, Portwing agent keys and name bindings, API keys with their scopes, audit entries, notification history and the notification outbox, image-restore backups, notification rules and the approval queue, the update-lifecycle and update-policy-retention caches, and containers live here; everything else still lives in `DD_STORE_FILE`, which still has to find your existing data on an upgrade. | filename | `dd.sqlite` |
