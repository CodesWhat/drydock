...
| `DD_STORE_DB_FILE` | ⚪ | SQLite database filename, stored alongside `DD_STORE_FILE` in the same directory. Nothing reads from it yet, so `DD_STORE_FILE` still means what it means today and still has to find your existing data. | filename | `dd.sqlite` |
