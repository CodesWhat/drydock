...
| `DD_UPDATE_OPERATION_ACTIVE_TTL_MS` | ⚪ | Maximum age (ms) an operation may remain in an `active` status (`queued` or `in-progress`) before it is expired to `failed`. Applied at startup and during housekeeping to clear stuck operations left behind by an unclean shutdown. | integer (`>0`) | `1800000` (30 min) |
