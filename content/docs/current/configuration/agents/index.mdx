...
That's the whole connection. The `{name}` segment (`REMOTE1`) is a label you choose; it's stored lowercased, so it appears as `remote1` in logs and the UI. A working connection logs `Handshake successful. Received N containers.` on the controller. If it doesn't connect, see [Troubleshooting](#troubleshooting). The rest of this page covers the full option set.
...
<Callout type="error">**Without this flag set, a secret configured over plain HTTP is a hard startup error — the controller refuses to boot.** You'll see:
...
Fix it by configuring TLS (`DD_AGENT_{name}_CAFILE`/`CERTFILE`), terminating TLS at a reverse proxy, or — on a trusted LAN only — setting `DD_AGENT_ALLOW_INSECURE_SECRET=true` (which demotes the error to a one-time warning).</Callout>
...
| `SSE Connection failed: connect ECONNREFUSED 192.168.1.50:3000. Retrying...` | Agent isn't running, or its port isn't published/reachable | Confirm the agent is up and its `ports:` are published; check firewalls |
...
If the **agent** container won't respond at all, check its own log for a startup error — the agent throws immediately if neither `DD_AGENT_SECRET` nor `DD_AGENT_SECRET_FILE` is set, and never starts listening.
