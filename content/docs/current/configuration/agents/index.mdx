...
<Callout type="warning">This becomes required, not optional, if an agent watches the same daemon a controller-local watcher already watches: the controller's own socket, or a controller-local TCP watcher pointed at the agent's host. The controller's watcher wins every new container on that daemon, and the agent's report of it is refused with a warning logged once per scan cycle. Set `DD_LOCAL_WATCHER=false` on the controller, or otherwise make sure only one side watches that daemon.</Callout>
