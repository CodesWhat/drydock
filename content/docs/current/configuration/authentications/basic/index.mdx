...
<Callout type="warn">**Known limitation:** Passwords containing colon characters (`:`) are not supported. The `Authorization: Basic` header parser splits the decoded credential on every colon and reads only the first two fields, so a password is truncated at its first colon and authentication fails. Use passwords without colons until this is resolved.</Callout>
...
<Callout type="warn">Legacy htpasswd hash formats from WUD/v1.3.x — `{SHA}` (SHA-1), `$apr1$` (Apache APR1-MD5), `$1$` (MD5-crypt), DES crypt, and plain text — are rejected as of v1.6. Use the commands above to generate an argon2id hash and update your `DD_AUTH_BASIC_*_HASH` values.</Callout>
