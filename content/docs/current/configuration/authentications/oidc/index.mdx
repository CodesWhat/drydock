...
<Callout type="info">Both `http://` and `https://` discovery URLs are supported. If your IdP runs behind a reverse proxy that terminates TLS, or on a private network without TLS, you can use an `http://` discovery URL. drydock automatically handles the insecure transport for the entire OIDC flow (discovery, token exchange, userinfo).</Callout>
