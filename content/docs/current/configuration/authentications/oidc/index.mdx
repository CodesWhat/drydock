...
<Callout type="warn">`http://` OIDC discovery URLs are automatically detected and allowed (openid-client v6 defaults to HTTPS-only). When the discovery URL uses `http:`, drydock passes `allowInsecureRequests` to the OIDC client. Migrate your IdP to HTTPS when possible for production deployments.</Callout>
