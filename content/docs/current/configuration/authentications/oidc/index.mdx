...
- **HTTPS discovery is required** — `http://` OIDC discovery URLs are rejected as of v1.6. Migrate your IdP to HTTPS; `INSECURE` only relaxes certificate verification for HTTPS and does not re-enable HTTP.
...
- **Node 24 compatibility** — `DD_AUTH_OIDC_{auth_name}_CAFILE` and `DD_AUTH_OIDC_{auth_name}_INSECURE=true` both require a custom TLS dispatcher. Drydock uses `fetch` from userland `undici` when a custom dispatcher is needed to avoid a Node 24 / undici v7 vs v8 dispatcher mismatch that caused opaque `TypeError: fetch failed` errors in prior releases. No configuration is needed — the fix is automatic.
- **Redirect URL validation** — Post-login redirects are validated against the backend's endpoint allowlist. OIDC flows cannot be hijacked into landing on arbitrary paths. No configuration is needed.
- **OIDC error log redaction** — OIDC warn logs redact RFC-1918 IP addresses (`[internal-addr]`) and absolute filesystem paths (`[path]`) from error cause chains so private network addresses and CA certificate file paths do not appear in centralized log aggregators.
...
Opt in to direct Authentik redirect only when you intentionally want to skip the internal login page. Add this option before the `...` line in the Docker command above:

```text
-e DD_AUTH_OIDC_AUTHENTIK_REDIRECT=true \
```
...
Opt in to a real Dex logout endpoint only when your IdP or auth proxy provides one. Add this option before the `...` line in the Docker command above:
