...
| `DD_OIDC_ALLOW_HTTP` | ⚪ | HTTP discovery compatibility override (`true` = explicit allow, `false` = explicit block) | `true`, `false` | unset (temporarily allowed with warning) |
...
<Callout type="warn">`http://` OIDC discovery is temporarily allowed when `DD_OIDC_ALLOW_HTTP` is unset (upgrade compatibility mode), and drydock logs warnings. To prepare for a future major release, set `DD_OIDC_ALLOW_HTTP=true` explicitly now and migrate your IdP to HTTPS as soon as possible. Set `DD_OIDC_ALLOW_HTTP=false` to fail-closed immediately.</Callout>
