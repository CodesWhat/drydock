...
### `DD_ANONYMOUS_CONFIRM`

Set `DD_ANONYMOUS_CONFIRM=true` to explicitly allow anonymous (unauthenticated) access to the dashboard and API.

```yaml
environment:
  - DD_ANONYMOUS_CONFIRM=true
```
...
| Scenario | Result |
| --- | --- |
| Fresh install, no auth configured, no `DD_ANONYMOUS_CONFIRM` | Fails closed — all API calls return `401` |
| Fresh install, `DD_ANONYMOUS_CONFIRM=true` | Anonymous access allowed |
| Upgrade from v1.3.x, no auth configured | Anonymous access allowed with a warning logged at startup |
| Upgrade from v1.3.x, `DD_ANONYMOUS_CONFIRM=true` | Anonymous access allowed, no warning |
...
2. **Explicitly opt in to anonymous access** — set `DD_ANONYMOUS_CONFIRM=true`.
