...
<Callout type="warn">Authentication protects all API routes and UI views unless anonymous access is enabled. Fresh installs must opt in with `DD_ANONYMOUS_AUTH_CONFIRM=true`; legacy upgrades without configured authentication retain anonymous access with a startup warning.</Callout>
