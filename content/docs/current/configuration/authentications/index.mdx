...
<Callout type="info">If a lockout value is missing or invalid, drydock falls back to the corresponding default.</Callout>
...
<Callout type="info">Use `DD_ANONYMOUS_AUTH_CONFIRM` exactly. Older names like `DD_ANONYMOUS_CONFIRM` and `DD_AUTH_ANONYMOUS_CONFIRM` are legacy and not used by current releases.</Callout>
