...
- Fully-pinned specific tags (3+ numeric segments, e.g. `1.4.5`, `v1.13.3`) also compare by digest by default, the same as floating tags, so a rebuilt image republished under the same tag is detected. Version climbing across tags for a pinned image is opt-in via `dd.tag.include` or `dd.tag.family=loose`. This doesn't leave you blind to what's out there, though: drydock still computes an informational `updateInsight` — the best newer same-family tag available in the registry, shown as a neutral "Newer available" badge in the UI — purely as information. It never triggers an update, a notification, or changes `updateAvailable`/`updateKind`; it only tells you a version exists so you can decide whether to opt into climbing to it. Unlike the actionable pin gate (strict family matching only), the informational insight allows crossing a MAJOR version — that's the point of surfacing it. Disable it per watcher with `DD_WATCHER_{watcher_name}_TAG_PIN_INFO=false`.
