...
The default `0 * * * *` (hourly) cron polls every container's registry once per hour. With 20+ watched containers and tag lists that paginate to thousands of entries (GHCR images can require 20+ paginated calls each), this can saturate anonymous Docker Hub limits (100 pulls / 6h) and trigger 429 responses. As of v1.5.0-rc.20 drydock automatically retries 429s with `Retry-After` honored and rate-limits its own outbound calls via a per-host token bucket, but if you watch many containers and don't need near-real-time detection, drop the cron to every 6 hours (`0 */6 * * *`) or once a day (`0 1 * * *`). New updates rarely ship more than once per day per image anyway.
</Callout>
