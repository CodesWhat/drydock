...
**Rate limiting.** A range fetch makes more GitHub API calls than two point lookups. Unauthenticated GitHub is capped at 60 requests per hour, so operators watching third-party images via `dd.source.repo` should configure `DD_RELEASE_NOTES_GITHUB_TOKEN` to make intermediate fetching viable.</Callout>
