...
As of **v1.5.0-rc.21** the default cron is `0 */6 * * *` (every 6 hours), matching Diun and the broader 2026 community consensus for self-hosted update managers. Earlier RCs defaulted to `0 * * * *` (hourly); if you have an explicit hourly cron set via `DD_WATCHER_{watcher_name}_CRON` it is preserved. With 20+ watched containers and tag lists that paginate to thousands of entries (GHCR images can require 20+ paginated calls each), hourly polling can saturate anonymous Docker Hub limits (100 pulls / 6h) and trigger 429 responses. drydock additionally retries 429s with `Retry-After` honored and rate-limits its own outbound calls via a per-host token bucket. If you need near-real-time detection (security patches), drop the cron to `0 * * * *`; if you want even quieter polling, use `0 1 * * *` (once daily). New updates rarely ship more than once per day per image anyway.
</Callout>
...
<Callout type="info">**Release notes inline popover** — The release-notes icon on each container row opens an inline popover showing both the current and available-version release notes side by side, with expand/collapse per panel. When only an external release URL is available (no structured notes), the popover contains a single row linking out to that URL. Release notes are fetched from GitHub via the release-notes provider using either a configured `DD_RELEASE_NOTES_GITHUB_TOKEN` or the GHCR token fallback (see [GHCR](/docs/configuration/registries/ghcr)) only when the source repository came from trusted image metadata, a GHCR image path, or a live Docker Hub metadata lookup. Operator/container-provided `dd.source.repo` values and persisted `container.sourceRepo` fallbacks are used anonymously so they cannot redirect your GitHub token to an arbitrary repository. Use `dd.source.repo` to supply a source repository when the image name does not map directly to a GitHub repo and anonymous release-note access is sufficient.</Callout>
...
The Containers list Version column adapts based on `tagPrecision`:
