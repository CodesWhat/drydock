...
A maintenance window restricts **when drydock is allowed to install an update on its own**. Outside the window, an update drydock would otherwise apply is deferred; drydock queues one pending check and applies the deferred update as soon as the window opens.
