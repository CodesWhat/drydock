...
| `DD_WATCHER_{watcher_name}_WATCHDIGEST` (deprecated) | ⚪ | If drydock must watch container digests instead of tags | `true`, `false` | not set |
...
<Callout type="warn">`DD_WATCHER_{watcher_name}_WATCHATSTART` is deprecated and will be removed in v1.6.0. Drydock watches at startup by default when unset or `true`; setting it explicitly to `false` still suppresses the initial startup scan today, but that will stop working once the variable is removed. Use `DD_WATCHER_{watcher_name}_CRON` to control scan timing instead.</Callout>
...
- Label-derived watcher fields (`tagFamily`, `includeTags`, `excludeTags`, `transformTags`, `actionTriggerInclude`, `actionTriggerExclude`, `notificationTriggerInclude`, `notificationTriggerExclude`, and the deprecated `triggerInclude`/`triggerExclude` mirror) are **re-derived on Docker update events** (e.g. after `docker compose up -d`). Labels set on the recreated container take effect immediately without waiting for the next cron cycle.
...
<Callout type="warn" title="Each label filters only its own trigger category">As of v1.6, `dd.action.include` / `dd.action.exclude` filter **only** action triggers (`docker`, `dockercompose`, `command`), and `dd.notification.include` / `dd.notification.exclude` filter **only** notification triggers (Slack, SMTP, Discord, and the rest). Setting one does not filter the other category — a container with only `dd.action.include` leaves every notification trigger unfiltered. To gate both categories, set both labels. The deprecated `dd.trigger.include` / `dd.trigger.exclude` labels still apply to both categories, but only for a category whose own scoped label is absent. Before v1.6 the two scoped labels collapsed into one internal field and whichever was set won for both categories ([#494](https://github.com/CodesWhat/drydock/issues/494)).</Callout>
