...
| `dd.depends_on` | ⚪ | Comma-separated list of container names this container depends on. Overrides compose-file `depends_on` detection entirely (no merge) — when set, drydock uses exactly this list. | CSV container names | Detected from the compose file's `depends_on` when the container is compose-managed; otherwise none |
| `dd.depends_on.action` | ⚪ | What to do with this container's dependents when it updates | `update`, `restart` | `update` |
