...
The hold applies to containers on the controller's own watchers. A container watched by a remote agent is not deferred yet: the container list and the update dialog show the window state the agent reports, but the controller does not hold its automatic updates back.
