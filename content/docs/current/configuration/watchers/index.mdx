...
| `DD_WATCHER_{watcher_name}_TAG_PIN_INFO` | ⚪ | Compute the informational `updateInsight` (best newer same-family tag) for pinned containers — see the pinned-tags note below | `true`, `false` | `true` |
...
- Fully-pinned specific tags (3+ numeric segments, e.g. `1.4.5`, `v1.13.3`) also compare by digest by default, the same as floating tags, so a rebuilt image republished under the same tag is detected. Version climbing across tags for a pinned image is opt-in via `dd.tag.include` or `dd.tag.family=loose`. This doesn't leave you blind to what's out there, though: drydock still computes an informational `updateInsight` — the best newer same-family tag available in the registry, shown as an informational "Newer available" badge in the UI — purely as information. It never triggers an update, a notification, or changes `updateAvailable`/`updateKind`; it only tells you a version exists so you can decide whether to opt into climbing to it. The insight reuses the same strict-style family matching used for actionable version climbing (prefix + suffix/variant compatibility + matching numeric-segment count) — that matching never restricted crossing a MAJOR version in the first place, pinned or not. What's new here is exposing the comparison at all for a pinned tag, since the pin gate itself already blocks acting on it. Disable it per watcher with `DD_WATCHER_{watcher_name}_TAG_PIN_INFO=false`.
...
<Callout type="info">
**Multi-network containers (macvlan, static IPs):** recreating a container attached to more than one network requires Drydock to call `POST /networks/{id}/connect` for each additional network after the replacement container is created. With Tecnativa/docker-socket-proxy, both `POST=1` and `NETWORKS=1` must be set — `NETWORKS=1` alone only covers reads. If you're using [sockguard](https://github.com/CodesWhat/sockguard) instead, it denies endpoint configs that specify a static IP or MAC address unless `request_body.network.allow_endpoint_config: true` is set in its policy — without it, reconnecting the extra networks during recreation fails.
</Callout>
