---
title: "Configuration"
description: "Configure Drydock with environment variables, Docker labels, and the v1.8 YAML configuration file."
---
...
| Config file | `drydock.yml` | `registry.ghcr.private.token` |
...
- [**Config file**](/docs/configuration/config-file) — `drydock.yml` beneath the environment, plus its Config API
- [**Backup & Rollback**](/docs/configuration/rollback) — automatic image backup and one-click restore
- [**Container Actions**](/docs/configuration/actions) — start, stop, restart, update, and delete from the UI/API
- [**Dashboard**](/docs/configuration/dashboard) — widget layout, customization, and responsive persistence
...
- [**Actions & Notifications**](/docs/configuration/triggers) — update-action providers, notification providers, event rules, templates, and routing
- [**UI / Appearance**](/docs/configuration/ui) — themes, icons, fonts, command palette, locale
- [**Update Bouncer**](/docs/configuration/security) — vulnerability scanning, signatures, and SBOM
- [**Watchers**](/docs/configuration/watchers) — Docker, Podman-compatible sockets, remote TLS, socket proxy security
- [**Webhooks**](/docs/configuration/webhooks) — CI/CD integration

## Diagnostics

Drydock includes a one-click diagnostic debug dump available in **Config > Diagnostics** or via the API at `GET /api/v1/debug/dump`. The dump collects a time-windowed snapshot of recent events, redacts values whose key names match the credential rules documented on the [app API page](/docs/api/app), and downloads as a JSON attachment. Downloads are audited and rate-limited to 5 per minute. Use the `?minutes=N` query parameter to control how many minutes of event history to include (default 30).

See [Diagnostic debug dump](/docs/api/app#download-diagnostic-debug-dump) for the full API reference.
...
services:
  ...
  # Valid semver followed by a build number (linux server style)
  duplicati:
