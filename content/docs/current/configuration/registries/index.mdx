...
| --- | --- | --- |
| `ghcr.io` | 2 req/s | 5 |
| `registry-1.docker.io` (Docker Hub) | 1 req/s | 3 |
...
The realm host is validated against the registry's own configured host before any credentials are sent — the exchange is silently skipped if the challenge points to an untrusted host, and the original 401 is returned instead. Providers that already perform proactive Bearer auth (GHCR, Docker Hub, Quay) are unaffected.
