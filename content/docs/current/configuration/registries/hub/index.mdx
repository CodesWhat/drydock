...
| `DD_REGISTRY_HUB_{REGISTRY_NAME}_PASSWORD` | ⚪ | A valid Docker Hub password or access token (`LOGIN` must be defined) | Docker Hub password or [access token](https://docs.docker.com/security/for-developers/access-tokens/) | |
| `DD_REGISTRY_HUB_{REGISTRY_NAME}_TOKEN` | ⚪ | A valid Docker Hub access token, alternative to `PASSWORD` (`LOGIN` must be defined) | Docker Hub [access token](https://docs.docker.com/security/for-developers/access-tokens/) | |
