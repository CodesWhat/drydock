...
| --- | :---: | --- | --- | --- |
| `DD_REGISTRY_HUB_{REGISTRY_NAME}_LOGIN` | ⚪ | A valid Docker Hub Login (required with `PASSWORD`) | Docker Hub username | |
...
| `DD_REGISTRY_HUB_{REGISTRY_NAME}_TOKEN` | ⚪ | **Deprecated** — use `PASSWORD` instead (accepted as an alias until v1.6.0) | Docker Hub [access token](https://docs.docker.com/security/access-tokens/) | |
...
<Callout type="warn">Registry auth now fails closed for invalid combinations. `LOGIN` must be paired with `PASSWORD`, and `AUTH` cannot be combined with `LOGIN`/`PASSWORD`/`TOKEN`. The `TOKEN` key is a deprecated alias for `PASSWORD` (removed in v1.6.0) — use `PASSWORD` for new configs.</Callout>
