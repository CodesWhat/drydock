...
<Callout type="warn">Legacy `wud.hook.*` labels are ignored as of v1.6. Rename them to `dd.hook.*` before upgrading, or use `node dist/index.js config migrate` to rewrite compose files.</Callout>
...
- **No slash in entry** (`sh`, `jq`): the basename of the hook command's first token is compared. `jq` matches both `jq` and `/usr/bin/jq`.
...
<Callout type="warn">`curl` was removed from the Docker image in v1.7.0 (see [Deprecations](/docs/deprecations#curl-healthcheck-override)). A hook that needs to make an HTTP call needs a mounted script or binary that provides its own HTTP client rather than a bare `curl` command.</Callout>
