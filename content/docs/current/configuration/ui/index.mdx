...
All UI preferences are configured in **Config > Appearance** and persisted in the browser's localStorage by default. No server-side environment variables are needed. Signed-in users can optionally turn on cross-device sync — see [Sync across devices](#sync-across-devices) below.
...
The dashboard is a customizable grid of 9 widgets — 4 stat cards and 5 detail widgets — with drag-to-reorder, resize, and per-widget visibility controls. v1.6 uses a zero-dependency CSS Grid implementation with natural same-row reorder, touch support, bounded resizing, and separate responsive layouts. See the dedicated [Dashboard guide](/docs/configuration/dashboard) for full details on widgets, edit mode, responsive breakpoints, and real-time updates.
...
The Agents toolbar offers **Normal spacing** (the default) and **Compact spacing**. Compact spacing reduces table-row padding without shrinking the row's interactive target below 44px. The choice is saved per browser and participates in **Sync across devices** when enabled. It does not change other views, column visibility or card layout; the control is hidden while cards are shown, and the saved spacing returns with the table.
...
To share grouping by a Docker label, select **Label** in the grouping control and enter the exact label key. The address updates automatically, for example `/containers?group-by-label=com.example.team`. Opening that link selects label grouping even if the browser previously used stack grouping. Label keys are case-sensitive; let the browser encode special characters rather than encoding the key before entering it.

If a link contains both `groupByStack=true` (or `1`) and `group-by-label`, explicit stack grouping wins. Switching grouping modes or clearing the label removes `group-by-label` without removing unrelated query fields. Navigating from a label-group link to a URL without that parameter clears label grouping; an empty parameter also clears it. A fresh page load without a grouping parameter keeps saved grouping preferences.
...
Translation strings are managed via [Crowdin](https://crowdin.com/project/drydock). Want to help translate Drydock into your language? See the [Translations guide](/docs/guides/translations).
...
<Callout type="info">UI preferences are stored in the browser's localStorage by default. Enable **Sync across devices** in Config > Appearance to also store them server-side and propagate changes to your other signed-in devices.</Callout>
