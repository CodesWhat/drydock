...
| `maturity-not-reached` | **Maturing** | Amber | Container's `maturityMode` is `mature` and the update has not yet aged past `maturityMinAgeDays` days. The gate measures elapsed time from the earliest known point the update existed: for Docker Hub and GHCR (including lscr.io), Drydock uses the registry's real publish timestamp (`last_updated` / `updated_at`), so an image already soaked in the registry longer than the threshold clears immediately on first detection. For all other registries (ACR, ECR, GCR, Quay, Harbor, etc.) the OCI image-config `created` field is a build date, not a reliable push date, so the gate falls back to `updateDetectedAt` — Drydock's own wall-clock time when it first recorded the pending update. The gate clock survives container recreation: a stack redeploy or `docker compose down && up` does not reset it as long as the same update is still pending. The tooltip shows days remaining and the exact `liftableAt` time. A trusted publish date that fails to parse or lies in the future (clock-skew protection) is treated as untrusted; the gate falls back to `updateDetectedAt` in that case. | Wait for the gate to lift (the tooltip shows the `liftableAt` timestamp), lower `maturityMinAgeDays`, or switch `maturityMode` to `all` from the **Update policy** menu. |
