...
| `maintenance-window-closed` | *no pill* | — | Reserved for automatic-update dispatch: recorded only when eligibility is evaluated with the maintenance window explicitly closed. No code path currently sets that flag — maintenance-window enforcement happens at scan time in the watcher, so manual UI/API updates are never gated by it and it isn't rendered as a pill. | N/A — manual updates are unaffected; automatic updates resume when the window reopens. |
