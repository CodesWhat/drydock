...
One case where the line overstates what happens: containers watched by a remote agent are not deferred by the controller yet. The dialog shows this blocker from the window state the agent reports, but the hold itself only applies to containers on the controller's own watchers.
