...
| Mode | Detection and notifications | Manual UI/API updates | Automatic action triggers | [Approval queue](/docs/configuration/actions/approvals) |
| --- | --- | --- | --- | --- |
| `notify` | Enabled | Refused | Suppressed | No rows — nothing is admissible to queue |
| `manual` | Enabled | Enabled, subject to eligibility | Suppressed | Every eligible candidate queues |
| `auto` | Enabled | Enabled, subject to eligibility | Enabled, subject to trigger configuration and eligibility | Only candidates a configured trigger will not auto-dispatch |
...
<Callout type="info">A soft blocker never hides a candidate from the [approval queue](/docs/configuration/actions/approvals) — it becomes one of the row's hold reasons instead, shown next to the release notes. Only a hard blocker keeps a candidate out (an existing row is left alone, since the blocker may still lift).</Callout>
...
The same hold applies to a container watched by a remote agent: the dialog reads the window state the agent reports, and the controller defers that container's automatic install the same way, catching up once the agent's next report shows the window open.
...
- [Approval Queue](/docs/configuration/actions/approvals) — the manual-mode candidate set as a work list, with the same hard/soft blocker split.
