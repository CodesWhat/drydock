...
<Callout type="warn">**Soft-blocker / hard-blocker change-ahead:** `dd.action.include` / `dd.action.exclude` are currently **soft blockers** — they suppress auto-updates but still allow manual updates (UI button, API) after confirmation. Later in the v1.7 cycle, shipping together with the per-action execution policy, these labels are scheduled to become **hard blockers** — containers with a non-matching include or an active exclude label will then also reject manual-update requests. Migrate your labels to `dd.action.include` / `dd.action.exclude` now so the flip can't lock you out.</Callout>
