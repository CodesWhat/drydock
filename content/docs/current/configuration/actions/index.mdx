...
<Callout type="info">Self-update operations (`drydock` updating itself) go through the same `withContainerUpdateLocks` path as regular container updates and are counted against the global cap. This is rarely relevant in practice since self-updates are infrequent.</Callout>
