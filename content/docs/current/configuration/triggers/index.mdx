...
| `DD_TRIGGER_*` | **Deprecated** — accepted in v1.6 with an `error`-level warning for every detected input; removed in v1.7.0 | `DD_TRIGGER_SLACK_MYSLACK_URL=https://...` |

Use `DD_ACTION_*` for update-action providers (Docker, Docker Compose, Command) and `DD_NOTIFICATION_*` for messaging providers (Slack, SMTP, Discord, etc.). The legacy `DD_TRIGGER_*` prefix still maps to the same configuration during the v1.6 compatibility window, but its error-level migration signal is intentionally loud; migrate it before v1.7.
