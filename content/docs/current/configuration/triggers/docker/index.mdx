...
<Callout type="warn">Hook commands are validated against a restricted shell-safe grammar at configuration time. Commands containing `$(...)`, backticks, redirections (`<`, `>`), or chaining metacharacters (`;`, `&`, `|`) are rejected with a clear error at startup. Allowed: a single command with arguments, single/double-quoted strings, and `$VAR` / `${VAR}` variable references.</Callout>
