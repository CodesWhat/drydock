...
6. Wait for the new container to become healthy (only when `dd.rollback.auto=true` and a `HEALTHCHECK` is configured)
...
When `dd.rollback.auto` is enabled, Drydock first waits for the recreated container to report `healthy` before it finalizes the update. After that, it continues polling the container health status at the configured interval. If the container becomes `unhealthy` within the monitoring window, an automatic rollback is triggered. If the window expires without failure, the container is considered stable.
