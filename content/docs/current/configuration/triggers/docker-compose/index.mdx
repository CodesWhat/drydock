...
The allowed roots are the directory holding the trigger's configured `FILE`, plus both sides of every bind mount Drydock itself has. The check runs on the final path: `config_files` values are mapped through Drydock's host-to-container bind mounts first, so a foreign host path that maps onto one of Drydock's own mounts is inside by construction. A path outside every root that the [mount-prefix fallback](#mount-prefix-fallback) recognises is replaced by the configured `FILE` rather than let through, so no consumer ever reads the path the container named. Paths the trigger itself is configured with are never checked.
