...
When [Update Bouncer](/docs/configuration/security) is configured, the compose trigger runs the full gate as a preflight, before any stop, remove, or create. It pulls the candidate image once, captures the exact repository digest that was pulled, and binds it to an immutable `repo:tag@digest` reference. Signature verification, the vulnerability scan, and SBOM generation all run against that pinned reference rather than the mutable tag, so a blocked image is never running and never written into the compose file. If preflight fails after the compose file has already been mutated for this update, the existing rollback restores the file before the error surfaces. In compose-file-once mode, every affected service is preflighted before the first runtime mutation for that batch, and a failed preflight terminalizes every queued operation in it.
