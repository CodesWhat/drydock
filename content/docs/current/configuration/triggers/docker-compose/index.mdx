...
The same policy decides what happens when a local tag moves between the pull and the create, which a concurrent `docker pull`, a second Drydock instance, or an operator retag can all do. Drydock inspects every container it creates and compares its image ID against the one the pull resolved to. On an ordinary compose update, `DD_SECURITY_AVAILABILITY_POLICY=warn` keeps the container and logs a warning naming both image IDs, while `block`, the default, removes the new container, restores the previous one, and fails the update. The policy is read whether or not a scanner is configured, so running without Update Bouncer and still wanting these updates to land means setting `DD_SECURITY_AVAILABILITY_POLICY=warn`. Compose-file-once mode ignores the policy and always refuses, because every container of the service is created against the one gate decision made in the preflight, and keeping a mismatched one would leave that container on an image the gate never cleared while its siblings run the image it did.
