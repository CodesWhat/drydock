...
The allowed roots are the directory holding the trigger's configured `FILE`, plus both sides of every bind mount Drydock itself has. The check runs on the final path: `config_files` values are mapped through Drydock's host-to-container bind mounts first, so a foreign host path that maps onto one of Drydock's own mounts is inside by construction, and a path the [mount-prefix fallback](#mount-prefix-fallback) rescues is inside too. Paths the trigger itself is configured with are never checked.
