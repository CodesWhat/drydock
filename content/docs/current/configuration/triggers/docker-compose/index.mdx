...
The identity the replicas are recreated from is normally the `repo:tag@sha256:...` reference the pull was bound to, so a tag that moves mid-batch cannot change what the second replica gets. When the daemon cannot bind that digest and `DD_SECURITY_AVAILABILITY_POLICY=warn` lets the update continue anyway, Drydock recreates every replica from the local image ID of the pull instead, which keeps them on one image without a digest to gate. The compose file still gets the tag either way.
