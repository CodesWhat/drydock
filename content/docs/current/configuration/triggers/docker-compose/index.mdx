...
| `DD_ACTION_DOCKERCOMPOSE_{trigger_name}_CONCURRENCY` | ⚪ | Max simultaneous compose-file groups updated through this trigger; overrides the global [`DD_UPDATE_CONCURRENCY`](/docs/configuration/actions#update-concurrency). Bounds how many independent compose files update at once — every container sharing one compose file still updates together as a single unit, in the same order it always has. | Positive integer | inherits the global value (`1`) |
...
### Label path containment

Compose paths taken from a container label (`dd.compose.file` or `com.docker.compose.project.config_files`) are contained to roots Drydock already knows about, so a container cannot point the trigger at an arbitrary file such as `/etc/hosts`.

Containment applies when Drydock runs in a container with at least one bind mount, or when the trigger has `FILE` configured. On a host run with no `FILE` and no bind mounts there is no known root to contain anything to, so label paths behave exactly as before.

The allowed roots are the trigger's configured `FILE` path and its parent directory, plus both sides of every bind mount Drydock itself has. The parent is left out when it is the filesystem root, so a `FILE` naming a top-level directory does not widen containment to the whole machine. The check runs on the final path: `config_files` values are mapped through Drydock's host-to-container bind mounts first, so a foreign host path that maps onto one of Drydock's own mounts is inside by construction. A path outside every root that the [mount-prefix fallback](#mount-prefix-fallback) recognises is replaced by the configured `FILE` rather than let through, so no consumer ever reads the path the container named. Paths the trigger itself is configured with are never checked. Containment only decides which file the action reads: when Drydock picks an action for a container, a rejected label still counts as the file the container named, so this trigger is not treated as a catch-all for containers that belong to another stack.

A label path outside every allowed root is dropped, with a warning naming the container, the label, the raw value, the resolved path and the allowed roots. The container is then handled as if the label had never been there, which means no compose file rather than the trigger's `FILE`: a container that asked for a file Drydock will not touch is skipped, not silently updated against a different stack.
...
What a mismatch costs depends on where that expected image ID came from. A compose-file-once preflighted update (see [Compose-file-once mode](#compose-file-once-mode)) resolves one identity per service before any container is touched and creates every container of the service against it, so a mismatch on a preflighted update is refused regardless of the availability policy: the new container is removed, the previous one is restored, and the update fails with both image IDs in the error. Everywhere else, including a compose-file-once update that reaches this check without having been preflighted, each container pulls and resolves its own image, there is no shared decision for one container to break, and `DD_SECURITY_AVAILABILITY_POLICY` decides. `block`, the default, refuses the same way. `warn` keeps the container, logs both image IDs, and, once the kept container starts successfully, records one `security-scan-skipped` audit entry naming them, which on an install with no scanner is the only record that anything happened. If the start then fails, the container is removed, the previous one is restored, and no audit entry claims the container was kept. That policy is read whether or not [Update Bouncer](/docs/configuration/security) is configured, so running without a scanner and still wanting these updates to land means setting `DD_SECURITY_AVAILABILITY_POLICY=warn`.
