...
<Callout type="warn">**Secret exposure:** The subprocess inherits the full drydock process environment, including all `DD_*` configuration variables (registry passwords, OIDC client secrets, agent tokens, etc.). Any command you run — including scripts invoked via `CMD` — can read these values. Treat the command trigger as a privileged operation: only point it at scripts you own and trust, and never pass user-controlled input through `CMD` without strict validation. If you need the command to operate without access to drydock credentials, launch it via a wrapper that `unset`s `DD_*` variables before delegating.</Callout>
