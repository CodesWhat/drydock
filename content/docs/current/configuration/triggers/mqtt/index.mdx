...
### Unreleased v1.7 state-topic fix

The maintenance backport of [#1139](https://github.com/CodesWhat/drydock/pull/1139) makes agent container state publish to the same topic advertised by Home Assistant discovery. With `HASS_ENABLED=true` and v1.7's unchanged `HASS_AGENTTOPICSEGMENT=true` default, an agent `edge`, watcher `local`, and container `nginx` now publish state to `dd/container/agent/edge/local/nginx`. Earlier v1.7 builds published to `dd/container/local/nginx` while discovery advertised the agent-scoped topic, which could leave the entity on `Unknown`. State reaches the corrected topic on the next container publish.

Controller-local topics, the explicit `HASS_AGENTTOPICSEGMENT=false` opt-out, and plain MQTT topics when `HASS_ENABLED=false` keep their existing layout. Container names still follow v1.7's canonical-name rules.

Old retained state messages on unscoped topics are not removed automatically. Move any custom subscribers for agent state to the corrected topic, and clear an old retained message only after checking that no controller-local container or other publisher still uses that topic. Any orphaned discovery entities from an earlier layout change still need the manual cleanup described above.
