...
| `DD_SECURITY_AVAILABILITY_POLICY` | ⚪ | Behavior when a scan cannot complete, and whether a compose update keeps a container whose local tag moved between the pull and the recreate ([Failure safety](/docs/configuration/triggers/docker-compose#failure-safety)). `block` is fail-closed; `warn` records an audit warning and permits the update. Read whether or not a scanner is configured, so it still governs the moved-tag case on an install with no Update Bouncer. Detected blocking findings always block. | `block`, `warn` | `block` |
...
| `DD_SECURITY_GATE_RELATIVE` | ⚪ | When an absolute severity threshold would block a candidate, allow it only if every severity count is less than or equal to the current image's saved scan. Missing, failed, or stale current-image scans remain blocked. | `true` / `false` | `false` |
