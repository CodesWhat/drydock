...
<Callout type="warn">**Removed in v1.7.0:** `curl` is no longer included in the Docker image. Custom `healthcheck:` overrides that shell out to curl (e.g. `test: curl --fail http://localhost:3000/health`) will fail. When Drydock can inspect its own container through the local Docker socket, it shows a UI deprecation banner and logs a startup warning naming the container if it detects a curl-based override. Use the built-in probe or switch to `/bin/healthcheck ${DD_SERVER_PORT:-3000}` for custom intervals.</Callout>
