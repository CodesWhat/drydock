...
| `DD_SESSION_SECRET` | 🔴 | **Required.** Persistent session secret for cookie signing. Must be a strong stable value (e.g. `openssl rand -hex 32`). The server refuses to start if unset. | Any string (≥32 chars recommended) | |
