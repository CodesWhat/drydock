...
| `DD_SERVER_UI_ENABLED` | ⚪ | Serve the web UI (set to `false` for headless/API-only mode). When disabled, UI paths answer with a JSON 404 body instead of Express's default HTML 404; `/api/*`, `/health`, and `/metrics` are unaffected. | `true`, `false` | `true` |
...
| `DD_SERVER_RATELIMIT_MAX` | ⚪ | Maximum requests accepted by the outer API limiter in each 15-minute window | integer (`>0`) | `1000` |
...
| `DD_AUTH_MAX_CONCURRENT_LOGIN_ATTEMPTS` | ⚪ | Maximum login credential verifications running at once. Excess attempts receive `429` before password hashing starts. | integer (`>0`) | `2` |
...
The unversioned `/api/*` alias was removed in v1.6.0, see [Deprecations](https://getdrydock.com/docs/deprecations). Setting `DD_COMPAT_WUDCARD=true` opts back in to a narrow slice of that surface: the four endpoints the Home Assistant [wud-card](https://github.com/angryvoegi/wud-card) integration calls. Three of them (the container list, per-container triggers list, and watch endpoints) are reshaped into the bare-array response that card expects instead of drydock's `{ data, total, limit, offset, hasMore, _links }` envelope; the fourth (running a trigger) already returns a small non-enveloped body and passes through unmodified. These endpoints stay mounted at `/api/*` even after the unversioned alias itself is removed, served by the compat router's own internal API router instance rather than by falling through to the removed alias. It exists purely to keep wud-card working against drydock and is off by default. Every other route under `/api/*` is unaffected by this flag; most of them now return `410 Gone`, except one standalone auth alias, `GET /api/auth/status`, that keeps responding unconditionally because it's registered directly on the app rather than through the removed alias router. `GET /api/auth/methods` was a similar standalone alias but was itself removed in v1.7.0, so it now falls through to the same `410 Gone` tombstone as the rest of the unversioned surface. See [Deprecations](https://getdrydock.com/docs/deprecations#unversioned-api-paths) for details.

The same flag also covers [Homepage](https://gethomepage.dev/)'s native `whatsupdocker` widget: Homepage's dashboard GETs `/api/containers` expecting the same bare array wud-card does, and proxies the request server-side, so no CORS configuration is needed for that integration.
...
<Callout type="warn">**Removed in v1.7.0:** `curl` is no longer included in the Docker image. Custom `healthcheck:` overrides that shell out to curl (e.g. `test: curl --fail http://localhost:3000/health`) will fail. When Drydock can inspect its own container through the local Docker socket, it shows a UI deprecation banner and logs a startup warning naming the container if it detects a curl-based override. Use the built-in probe or switch to `/bin/healthcheck $${DD_SERVER_PORT:-3000}` for custom intervals (the doubled `$` is compose escaping, so the variable resolves inside the container instead of being expanded from the host environment before the container starts).</Callout>
...
- **WebSocket connection failures** — log streaming and container log streaming upgrades are rejected because drydock can't confirm the browser's Origin scheme matches
