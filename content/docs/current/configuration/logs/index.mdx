...
On interactive terminals, Drydock prints a color startup banner with the version and mode. The banner is automatically suppressed when stderr is not a TTY and when `NO_COLOR` is set to a non-empty value, so structured logs and piped output stay clean.
...
The browser keeps up to 2,000 live application-log entries for sorting, search, and virtualization. When that client buffer fills, the oldest row is discarded and the collection rolls forward immutably, so newest-first views continue advancing as new WebSocket frames arrive.
