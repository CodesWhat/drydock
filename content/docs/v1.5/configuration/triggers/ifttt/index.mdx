...
| --- | :---: | --- | --- | --- |
| `DD_NOTIFICATION_IFTTT_{trigger_name}_KEY` | 🔴 | The Webhook key | | |
| `DD_NOTIFICATION_IFTTT_{trigger_name}_EVENT` | ⚪ | The event name | | `dd-image` |
...
When `MODE=batch`, the webhook behavior depends on the event context:

- **Standard batch:** `value1` contains the full JSON array of all containers with available updates. `value2` and `value3` are not sent.
- **Security-alert digest** (when a `runtimeContext` with `title`/`body` is present, e.g. `security-alert-digest` events): `value1` = alert title, `value2` = alert body, `value3` = JSON array of affected containers.
...
services:
  drydock:
    ...
    ...
    environment:
      - DD_NOTIFICATION_IFTTT_PROD_KEY=*******************************************
```
...
```bash
docker run \
  -e DD_NOTIFICATION_IFTTT_PROD_KEY="*******************************************" \
  ...
