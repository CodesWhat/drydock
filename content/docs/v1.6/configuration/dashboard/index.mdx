...
| Widget | Shows |
| --- | --- |
| **Recent Updates** | Pending container updates with update/skip action buttons in a scrollable list. Includes shared release-notes and **View project** links, plus an "Update All" batch button when pending updates exist. Registry failures are excluded. |
...
| **Update Breakdown** | Four-column grid showing major, minor, patch, and same-tag **image update** counts with color-coded pills and proportional progress bars. |
...
The v1.6 grid is implemented directly with CSS Grid rather than `grid-layout-plus`. Reordering repacks widgets into the earliest available cells, including natural same-row moves; mouse and touch drag gestures show the active drop target, and resize handles clamp every widget to its documented minimum and maximum size.

All changes are saved automatically to `localStorage` and persist across page reloads. Signed-in users who enable **Sync across devices** in **Config > Appearance** also store dashboard layouts server-side with the rest of their UI preferences.
...
Container-change events patch the affected dashboard data and refresh the summary. Security-scan completion does not force a separate full-dashboard refresh; the security surfaces update through their own data flow.

The **Resource Usage** widget has a dedicated stats SSE stream (`GET /api/v1/stats/summary/stream`) that pushes `dd:stats-summary` events on every aggregator tick (default 10 s). The widget connects to this stream independently of the main UI event channel, so resource stats update even when no container state changes occur.
...
<Callout type="info">Dashboard preferences are per-browser by default. Signed-in users can opt into per-account cross-device synchronization in **Config > Appearance**; anonymous sessions remain local-only.</Callout>
