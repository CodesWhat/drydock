...
| --- | :---: | --- | --- | --- |
| `DD_NOTIFICATION_GOTIFY_{trigger_name}_PRIORITY` | ⚪ | The Gotify message priority | Integer greater or equal than `0` | |
| `DD_NOTIFICATION_GOTIFY_{trigger_name}_TOKEN` | 🔴 | The Gotify app token | A valid gotify app token | |
| `DD_NOTIFICATION_GOTIFY_{trigger_name}_URL` | 🔴 | The Gotify server url | The `http` or `https` gotify server address | |
...
services:
  drydock:
    ...
    ...
    environment:
      - DD_NOTIFICATION_GOTIFY_LOCAL_URL=http://gotify.localhost
      - DD_NOTIFICATION_GOTIFY_LOCAL_TOKEN=your-gotify-app-token
```
...
```bash
docker run \
  -e DD_NOTIFICATION_GOTIFY_LOCAL_URL="http://gotify.localhost" \
  -e DD_NOTIFICATION_GOTIFY_LOCAL_TOKEN="your-gotify-app-token" \
  ...
  codeswhat/drydock
```
</Tab>
